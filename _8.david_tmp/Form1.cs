...
namespace iMS_Link
{
    public partial class Form1 : Form
    {
        ...
        bool flag_show_brightness = false;
        ...
        string iMS_Link_log_filename = Application.StartupPath + "\\log\\iMS_Link.log";
        string RxString = "";
        ...
        bool flag_ims_egd_exist = false;
        int connect_comport_cnt_max = 0;
        ...
        void Write_Log_File(string input)
        {
            ...
            if (log_file_tmp_length > 100)
            {
                ...
                {
                    //richTextBox1.Text += "檔案: " + iMS_Link_log_filename + " 存在, 開啟，並接續寫入資料。\n";
                    StreamWriter sw = File.AppendText(iMS_Link_log_filename);
                    sw.Write(log_file_tmp_data);
                    sw.Close();
                }
        ...
        public Form1()
        {
            ...
            g11 = panel_hipot.CreateGraphics();
            g11.Clear(BackColor);
            g11.DrawString("先填狀態", new Font("標楷體", 60), new SolidBrush(Color.Green), new PointF(15, 20));

            g12 = panel_cosmo.CreateGraphics();
            g12.Clear(BackColor);
            g12.DrawString("輸入相機序號", new Font("標楷體", 43), new SolidBrush(Color.Green), new PointF(1, 30));

            Reset_iMS_Link_Setting();
        ...
        {
            richTextBox1.Text += "你按了按鈕 Name : " + ((Button)sender).Name + "\n";
            flag_save_measure_color_picture = true;

            if (flag_use_metering_color == false)
                measure_picture();
        }
        ...
        private void Reset_iMS_Link_Setting()
        {
            ...
            groupBox14.Size = new Size(gb_w, gb_h);
            groupBox15.Size = new Size(gb_w, gb_h);
            groupBox16.Size = new Size(gb_w, gb_h);
            groupBox17.Size = new Size(gb_w, gb_h);
            groupBox18.Size = new Size(gb_w, gb_h);
            groupBox19.Size = new Size(gb_w, gb_h);

            groupBox1.Location = new Point(x_st, y_st);
            groupBox4.Location = new Point(x_st, y_st);
            groupBox5.Location = new Point(x_st, y_st);
            groupBox10.Location = new Point(x_st, y_st);
            groupBox14.Location = new Point(x_st, y_st);
            groupBox15.Location = new Point(x_st, y_st);
            groupBox16.Location = new Point(x_st, y_st);
            groupBox17.Location = new Point(x_st, y_st);
            groupBox18.Location = new Point(x_st, y_st);
            groupBox19.Location = new Point(x_st, y_st);
            ...
            groupBox11.Size = new Size(gb_w, gb_h);     //連線-相機狀態
            groupBox3.Size = new Size(gb_w, gb_h);      //連線-主機時間
            groupBox11.Location = new Point(x_st, y_st);

            y_st = 260;
            groupBox3.Location = new Point(x_st, y_st);

            panel1.Location = new Point(x_st + gb_w - 160, y_st + gb_h + 20);
            ...
            btn1.Text = "Play";
            btn1.Name = "bt_play";
            btn1.Location = new Point((256 * 2 + margin * 2) / 2 + 50, (256 * 2 + margin * 2) - btn1.Height - 5);
            // 加入按鈕事件
            //btn1.Click += new EventHandler(myClick1);   //same
            btn1.Click += myClick1;
            // 將按鈕加入表單
            ...
            btn2.Text = "Save";
            btn2.Name = "bt_save";
            btn2.Location = new Point((256 * 2 + margin * 2) / 2 + 50 + 100, (256 * 2 + margin * 2) - btn2.Height - 5);
            // 加入按鈕事件
            //btn2.Click += new EventHandler(myClick2);   //same
            btn2.Click += myClick2;
            // 將按鈕加入表單
            ...
            btn3.Text = "X";
            btn3.Name = "bt_close";
            btn3.Location = new Point(10, 10);
            // 加入按鈕事件
            //btn3.Click += new EventHandler(myClick3);   //same
            btn3.Click += myClick3;
            // 將按鈕加入表單
            ...
            hsbar.Location = new Point(50, (256 * 2 + margin * 2) - 40);

            // 加入按鈕事件
            //hsbar.Scroll += new EventHandler(myScroll);   //same
            hsbar.Scroll += myScroll;

            // 將按鈕加入表單
            this.pictureBox3.Controls.Add(hsbar);
            ...
            if (flag_operation_mode != MODE_RELEASE_STAGE0)
            {
                ...
                else if ((flag_operation_mode == MODE_RELEASE_STAGE1C) || (flag_operation_mode == MODE_RELEASE_STAGE5) || (flag_operation_mode == MODE_RELEASE_STAGE6)
                    || (flag_operation_mode == MODE_RELEASE_STAGE9) || (flag_operation_mode == MODE_RELEASE_STAGE11) || (flag_operation_mode == MODE_RELEASE_STAGE12)
                    || (flag_operation_mode == MODE_RELEASE_STAGE20))
                {
            ...
            if ((flag_operation_mode == MODE_RELEASE_STAGE1A) || (flag_operation_mode == MODE_RELEASE_STAGE1B))
            {
                ...
                this.tp_Camera_Test.Parent = null;  //群光相機檢查
                this.tp_Check.Parent = null;    //檢查結果
                this.tp_Cosmo.Parent = null;    //COSMO氣密測試
                this.tp_HiPot.Parent = null;    //Hi-Pot紀錄
                this.tp_Temperature.Parent = null;  //CPU 溫度顯示
                this.tp_Test.Parent = null;     //Test
                this.tp_Layer.Parent = null;    //Layer
            ...
            else if (flag_operation_mode == MODE_RELEASE_STAGE1C)
            {
                ...
                this.tp_Test.Parent = null;     //Test
                this.tp_Layer.Parent = null;    //Layer
                tabControl1.SelectTab(tp_Check);    //程式啟動時，直接跳到指定分頁。
                timer_rtc.Enabled = false;
            ...
            else if (flag_operation_mode == MODE_RELEASE_STAGE2)
            {
                ...
                this.tp_HiPot.Parent = null;    //Hi-Pot紀錄
                this.tp_Temperature.Parent = null;  //CPU 溫度顯示
                //tabControl1.SelectedTab = tp_Connection;  //程式啟動時，直接跳到指定分頁。
                tabControl1.SelectTab(tp_Connection);       //程式啟動時，直接跳到指定分頁。   the same
            ...
            else if (flag_operation_mode == MODE_RELEASE_STAGE5)
            {
                ...
                this.tp_HiPot.Parent = null;    //Hi-Pot紀錄
                this.tp_Temperature.Parent = null;  //CPU 溫度顯示
                tabControl1.SelectTab(tp_Product);  //程式啟動時，直接跳到指定分頁。
                timer_rtc.Enabled = false;
                timer_rgb.Enabled = false;
            ...
            else if (flag_operation_mode == MODE_RELEASE_STAGE6)
            {
                ...
                this.tp_Test.Parent = null;     //Test
                this.tp_Layer.Parent = null;    //Layer
                tabControl1.SelectTab(tp_Package1);  //程式啟動時，直接跳到指定分頁。
                timer_rtc.Enabled = false;
                timer_rgb.Enabled = false;
                timer_stage5.Enabled = false;
                timer_stage1.Enabled = false;
            ...
            else if (flag_operation_mode == MODE_RELEASE_STAGE9)
            {
                ...
                this.tp_Check.Parent = null;
                this.tp_Cosmo.Parent = null;
                this.tp_HiPot.Parent = null;    //Hi-Pot紀錄
                this.tp_Temperature.Parent = null;  //CPU 溫度顯示
                this.tp_Camera_Test.Parent = null;  //群光相機檢查
                tabControl1.SelectTab(tp_sale);  //程式啟動時，直接跳到指定分頁。
                timer_rtc.Enabled = false;
                timer_rgb.Enabled = false;
                timer_stage1.Enabled = false;
                timer_stage5.Enabled = false;
            ...
            else if (flag_operation_mode == MODE_RELEASE_STAGE11)
            {
                ...
                this.tp_Temperature.Parent = null;  //CPU 溫度顯示
                this.tp_Camera_Test.Parent = null;  //群光相機檢查
                this.tp_sale.Parent = null;
                tabControl1.SelectTab(tp_HiPot);  //程式啟動時，直接跳到指定分頁。
                timer_rtc.Enabled = false;
                timer_rgb.Enabled = false;
                timer_stage1.Enabled = false;
                timer_stage5.Enabled = false;
            ...
            else if (flag_operation_mode == MODE_RELEASE_STAGE12)
            {
                ...
                this.tp_sale.Parent = null;     //出貨紀錄
                this.tp_Camera_Test.Parent = null;  //群光相機檢查
                tabControl1.SelectTab(tp_Cosmo);  //程式啟動時，直接跳到指定分頁。
                timer_rtc.Enabled = false;
                timer_rgb.Enabled = false;
                timer_stage5.Enabled = false;
                timer_stage1.Enabled = false;
            ...
            else if (flag_operation_mode == MODE_RELEASE_STAGE20)
            {
                ...
                this.tp_Temperature.Parent = null;  //CPU 溫度顯示
                this.tp_sale.Parent = null;     //出貨紀錄
                tabControl1.SelectTab(tp_Camera_Test);  //程式啟動時，直接跳到指定分頁。
                timer_rtc.Enabled = false;
                timer_rgb.Enabled = false;
                timer_stage5.Enabled = false;
                timer_stage1.Enabled = false;
            ...
            button33.BackgroundImage = iMS_Link.Properties.Resources.open_log;
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            richTextBox1.AppendText("[PC] : Reset iMS_Link\n");
            richTextBox1.ScrollToCaret();       //RichTextBox顯示訊息自動捲動，顯示最後一行
            Reset_iMS_Link_Setting();
        }
        ...
        private void Comport_Scan()
        {
            ...
            comboBox_comport3.Items.Clear();    //Clear All items in Combobox
            ...
            foreach (string port in COM_Ports_NameArr)
            {
                ...
                comboBox_comport3.Items.Add(port);
            }
            richTextBox1.Text += "\n";
            ...
            if (COM_Ports_NameArr.Length > 0)
            {
                ...
                comboBox_comport3.Text = COM_Ports_NameArr[0];
            }

            if (COM_Ports_NameArr.Length >= 2)
            {
                comboBox_comport1.Text = COM_Ports_NameArr[COM_Ports_NameArr.Length - 2];   //倒數第2個
                comboBox_comport3.Text = COM_Ports_NameArr[COM_Ports_NameArr.Length - 2];   //倒數第2個
            }
        }
        ...
        {
            groupBox_comport1.Location = new Point(0, 0);
            ...
            {
                groupBox_quick.Location = new Point(850 + 10, 5);
                groupBox_quick.Size = new Size(670, 60);
            ...
            lb_yuv_u.Location = new Point(dx * 0 + 45, y_st + dy * 6 + 40 + 25 * 4 + 50);
            lb_yuv_v.Location = new Point(dx * 0 + 90, y_st + dy * 6 + 40 + 25 * 4 + 50);
            ...
            */
            ...
            if (flag_display_mode == DISPLAY_SD)
            {
                    ...
                    || (flag_operation_mode == MODE_RELEASE_STAGE2) || (flag_operation_mode == MODE_RELEASE_STAGE3))
                {
                    ...
                    groupBox_sn1.Location = new Point(140, 400 + 15);
                    groupBox_sn1.Size = new Size(230, 170);
                    lb_sn_opal.Location = new Point(5, 10);
                    tb_sn_opal.Location = new Point(5, 10 + 37 * 1);
                    tb_wait_sn_data.Location = new Point(5, 10 + 37 * 2);
                    lb_main_mesg2.Location = new Point(5, 10 + 37 * 3);
                    lb_class.Location = new Point(270, 412 + 15);
                }
            }
            else
            {
            ...
            else
            {
                ...
                groupBox_comport1.Visible = false;
            ...
            if (flag_operation_mode == MODE_RELEASE_STAGE3)
            {
                ...
                checkBox_only_picture.Visible = true;
            }
            else
            {
                lb_class.Visible = false;
                checkBox_only_picture.Visible = false;
            }
    ...
    || (flag_operation_mode == MODE_RELEASE_STAGE1A) || (flag_operation_mode == MODE_RELEASE_STAGE1B) || (flag_operation_mode == MODE_RELEASE_STAGE3))
                    ...
                    if (flag_ims_egd_exist == false)
                    {
                        if ((DateTime.Now.Second % 5) == 0)
                        {
                            if (check_ims_camera() == S_OK)
                            {
                                ...
                                show_main_message1("有ims相機", S_OK, 30);
                                show_main_message2("有ims相機", S_OK, 30);
                                show_main_message3("有ims相機", S_OK, 30);
                                tb_awb_mesg.BackColor = Color.White;
                                tb_awb_mesg.Text = "";
                            }
                            else
        ...
        private void button33_Click(object sender, EventArgs e)
        {
            if (show_comport_log == false)
            {
                ...
                button33.BackgroundImage = iMS_Link.Properties.Resources.close_log;
        ...
        */
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            lb_write_camera_serial2.Text = "有連接器, 有相機";
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                richTextBox1.Text += "進入 USB WebCam 頁\t";
            ...
            else if (tabControl1.SelectedTab == tp_Serial_Auto)
            {
                ...
                richTextBox1.Text += "改成兩筆序號正確即進行燒錄\n";
                tb_sn1.Text = "兩筆序號正確即進行燒錄";
                timer_rgb.Enabled = false;
                timer_stage1.Enabled = false;
                timer_stage2.Enabled = false;
                timer_stage3.Enabled = false;
                ...
                groupBox_quick.Location = new Point(613 + 10, 0);
            }
            else if (tabControl1.SelectedTab == tp_Product)
            {
                richTextBox1.Text += "進入 產品包裝 頁, 第5站\n";
                timer_rgb.Enabled = false;
                timer_stage1.Enabled = false;
                timer_stage2.Enabled = false;
                timer_stage3.Enabled = false;
            ...
            {
                richTextBox1.Text += "進入 Dongle 頁, 第7站\n";
            ...
            {
                richTextBox1.Text += "進入 主機包裝 頁, 第8站\n";
            ...
            {
                richTextBox1.Text += "進入 出貨記錄 頁, 第9站\n";
            ...
            else if (tabControl1.SelectedTab == tp_HiPot)
            {
                richTextBox1.Text += "進入 Hi Pot記錄 頁, 第11站\n";
                ...
                tb_hipot_status.Clear();
                tb_sn_opal_hipot.Clear();

                tb_hipot_status.BackColor = Color.Pink;
                tb_sn_opal_hipot.BackColor = Color.White;
            ...
            {
                richTextBox1.Text += "進入 檢查結果 頁, 第1C站\n";
            ...
            else
            {
                ...
                if (tabControl1.SelectedTab == tp_Camera)
                {
                    richTextBox1.Text += "進入 Camera 頁\n";
                }
                else if (tabControl1.SelectedTab == tp_Info)
                {
                    richTextBox1.Text += "進入 相機資料 頁\n";
                }
                else if (tabControl1.SelectedTab == tp_Connection)
                {
                    richTextBox1.Text += "進入 連線 頁\n";
                }
                else if (tabControl1.SelectedTab == tp_System)
                {
                    richTextBox1.Text += "進入 主機序號 頁\n";
                }
                else if (tabControl1.SelectedTab == tp_Camera_Model)
                {
                    richTextBox1.Text += "進入 相機型號 頁\n";
                }
                else if (tabControl1.SelectedTab == tp_Test)
                {
                    richTextBox1.Text += "進入 Test 頁\n";
                }
                else if (tabControl1.SelectedTab == tp_Layer)
                {
                    richTextBox1.Text += "進入 Layer 頁\n";
                }
                else if (tabControl1.SelectedTab == tp_Temperature)
                {
                    richTextBox1.Text += "進入 CPU溫度偵測 頁\n";
                }
                else
                {
        ...
        void Cam_NewFrame(object sender, NewFrameEventArgs eventArgs)
        {
            ...
            if ((flag_enaglb_awb_function == true) || (flag_check_webcam_signal == true))
            {
                ...
                if (flag_show_brightness == true)
                {
                    if ((frame_cnt % 33) == 0)
                    {
                        show_picture_brightness_histogram();
                    }
                }
            }
        ...
        private void timer_stage4_Tick(object sender, EventArgs e)
        {
            ...
            if (tb_wait_camera_data.Text.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        lb_write_camera_serial2.Text = "直接燒錄";
                        lb_write_camera_serial2.ForeColor = Color.Black;
                        richTextBox1.Text += "直接燒錄1\n";
                        flag_ok_to_write_data = true;
                        panel9.BackgroundImage = iMS_Link.Properties.Resources.ims_egd_system;

                        tb_wait_camera_data.Text = "IMS EGD SYSTEM";
        ...
        {
            richTextBox1.Text += "你按了 ZOOM flag_fullscreen = " + flag_fullscreen.ToString() + "\n";
            ...
            if (flag_fullscreen == false)
            {
                ...
                if (flag_display_mode == DISPLAY_SD)
                {
                    //SD + big
                    /*  useless
                    richTextBox1.Text += "bt_zoom_Click skip setting\n";
                    int dy = 0;
                    lb_sn_opal.Text = "序號";
                ...
                else
                {
                        ...
                        || (flag_operation_mode == MODE_RELEASE_STAGE2) || (flag_operation_mode == MODE_RELEASE_STAGE3))
                    {
                        ...
                        lb_sn_opal.Location = new Point(5, 10);
                        tb_sn_opal.Location = new Point(5, 10 + 30);
                        tb_wait_sn_data.Location = new Point(5, 10 + 30 * 2);
                        lb_main_mesg2.Location = new Point(5, 10 + 30 * 3);

                        lb_class.Location = new Point(160, 675 - 30 - 20 - 10 - 3);
                ...
                bt_zoom.BackgroundImage = iMS_Link.Properties.Resources.normal_screen;
                //this.TopMost = true;
            ...
            else
            {
                ...
                else
                {
                    ...
                    groupBox_quick.Location = new Point(613 + 5, 0);
                }
        ...
        private void button24_Click(object sender, EventArgs e)
        {
            ...
            richTextBox1.Text += "Camera頁 裡面的 相機型號 讀取\n";
            byte page;
        ...
        {
            richTextBox1.Text += "Camera頁 裡面的 相機型號 寫入\n";
        ...
        {
            richTextBox1.Text += "測試頁 裡面的 相機寫任意序號 按鈕\n";
            ...
            if (tb_wait_data.Text.Length > 0)
            {
                ...
                else
                {
                    ...
                    if ((flag_ok_machine_serial == true) && (flag_ok_mb_big_serial == true) && (flag_ok_mb_small_serial == true))
                    {
                        ...
                        flag_ok_to_write_data = true;
                        panel6.BackgroundImage = iMS_Link.Properties.Resources.ims_egd_system;
                    }
                }
                tb_wait_data.Text = "";
            }
        ...
        private void timer_stage2_Tick(object sender, EventArgs e)
        {
            ...
            */
            ...
            {
                tb_sn_opal.Clear();
                show_main_message1("無ims相機", S_FALSE, 30);
                show_main_message2("無ims相機", S_FALSE, 30);
                show_main_message3("無ims相機", S_FALSE, 30);
        ...
        int do_awb(object sender, EventArgs e)
        {
            ...
            richTextBox1.Text += "檢查UART連線\n";
            if (get_comport_status() == false)
                return REASON_NO_COMPORT;

            richTextBox1.Text += "檢查相機連線\n";
            g_conn_status = get_camera_status();
            ...
            {
                //MessageBox.Show("No Insighteyes Camera", "iMS_Link", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        int try_connect_comport()
        {
            ...
            if (COM_Ports_NameArr.Length == 1)
            {
                ...
                ret = connect_comport(comboBox_comport1.Text);
            }
            else
            {
                ...
                for (int i = 0; i < COM_Ports_NameArr.Length; i++)
                {
                    ...
                    comboBox_comport1.Text = COM_Ports_NameArr[try_index];
                    comboBox_comport3.Text = COM_Ports_NameArr[try_index];
            ...
            finally
            {
                ...
                if (serialPort1.IsOpen)
                {
                    ...
                    */
                    delay(50);
                    serialPort1.DiscardInBuffer();  //丟棄UART buffer內的資料
                    SerialPortTimer100ms.Stop();
                    SerialPortTimer100ms.Start();
                    Application.DoEvents();
                    delay(100);
                    ...
                    int cnt = 0;
                    while ((flag_comport_connection_ok == false) && (cnt++ < 40))
                    {
                        //richTextBox1.Text += "x";
                        delay(10);
                        /*
                    ...
                    richTextBox1.Text += "\ncnt = " + cnt.ToString() + "\n";    //usually cnt = 4
                    if (cnt > 30)
                        richTextBox1.Text += "XXXXXXXXXXXXX\n";

                    if ((cnt > connect_comport_cnt_max) && (cnt < 41))
                    {
                        connect_comport_cnt_max = cnt;
                        richTextBox1.Text += "let cnt = " + cnt.ToString() + "\n";
                    ...
                    richTextBox1.Text += "時間 : " + diff_time.ToString() + "\n";
                    flag_comport_ok = false;
        ...
        void exportCSV()
        {
            ...
            */
            ...
            for (i = 0; i < camera_serials.Count; i++)
            {
                ...
                {
                    csv_index11++;
                    content += csv_index11.ToString() + "," + camera_serials[i][0].ToString() + "," + camera_serials[i][1].ToString() + "," + camera_serials[i][2].ToString();
                    csv_index11 -= camera_serials.Count;
                }
                else if (flag_operation_mode == MODE_RELEASE_STAGE12)
                {
                    csv_index12++;
        ...
        */
        ...
        void connect_IMS_comport()
        {
            ...
            if (ret == S_OK)
            {
                ...
                pictureBox_comport.Image = iMS_Link.Properties.Resources.comport;
        ...
        void show_main_message11(string mesg, int number, int timeout)
        {
            lb_main_mesg11.Text = mesg;
            playSound(number);
        ...
        int check_tb_sn_opal_data11()
        {
            ...
            len = tb_wait_hipot_data.Text.Length;

            if (len > 20)   //太長, 直接放棄
            {
                tb_wait_hipot_data.Clear();
                flag_incorrect_data = true;
                return S_FALSE;
            }
            else if (len > 5)    //檢查是否換行
            {
                if ((tb_wait_hipot_data.Text[len - 2] == 0x0D) || (tb_wait_hipot_data.Text[len - 1] == 0x0A))
                {
                    tb_wait_hipot_data.Text = tb_wait_hipot_data.Text.Trim();
                    //richTextBox1.Text += "OK";
                }
                else
                {
            ...
            if (tb_wait_hipot_data.Text.Length == 0)
            {
                //richTextBox1.Text += "未輸入資料\n";
                flag_incorrect_data = true;
            ...
            else if ((tb_wait_hipot_data.Text.Length == 9) || (tb_wait_hipot_data.Text.Length == 10))
            {
                //檢查英文字母的正確性
                if (((tb_wait_hipot_data.Text[0] >= 'A') && (tb_wait_hipot_data.Text[0] <= 'Z')) || ((tb_wait_hipot_data.Text[0] >= 'a') && (tb_wait_hipot_data.Text[0] <= 'z')))
                {
                    flag_incorrect_data = false;
                }
                else
                ...
                if (((tb_wait_hipot_data.Text[1] >= 'A') && (tb_wait_hipot_data.Text[1] <= 'Z')) || ((tb_wait_hipot_data.Text[1] >= 'a') && (tb_wait_hipot_data.Text[1] <= 'z')))
                {
                    flag_incorrect_data = false;
                }
                else
                ...
                for (i = 2; i < tb_wait_hipot_data.Text.Length; i++)
                {
                    if ((tb_wait_hipot_data.Text[i] < '0') || (tb_wait_hipot_data.Text[i] > '9'))
                    {
                        flag_incorrect_data = true;
                        richTextBox1.Text += "SN1格式不正確b\n";
                    }
                ...
                if (flag_incorrect_data == false)
                {
                    richTextBox1.Text += "c取得 SN1序號 : " + tb_wait_hipot_data.Text + "\n";
                }
            }
            else
            {
            ...
            if (flag_incorrect_data == true)
            {
                richTextBox1.Text += "資料錯誤,長度 " + tb_wait_hipot_data.Text.Length.ToString() + "\t內容 " + tb_wait_hipot_data.Text + "\t清除\n";
        ...
        private void timer_stage5_Tick(object sender, EventArgs e)
        {
            ...
            {
                bool flag_incorrect_data = false;
                if (tb_wait_product_data.Text.Length == 12) //支持1英11數
                {
                    for (i = 1; i < tb_wait_product_data.Text.Length; i++)
                    {
                    ...
                    {
                        //檢查英文字母的正確性
                        if ((tb_wait_product_data.Text[0] < 'A') || (tb_wait_product_data.Text[0] > 'Z'))
                        {
                            flag_incorrect_data = true;
                            richTextBox1.Text += "資料長度正確, 數字正確, 但是英文字錯誤, 清除\n";
                            tb_wait_product_data.Text = "";
                            tb_product1.Clear();
                            tb_product1.BackColor = Color.Pink;
                        }
                ...
                else if (tb_wait_product_data.Text.Length == 11)    //因應重工 要能支持11數
                {
                ...
                else if (tb_wait_product_data.Text.Length == 20)
                {
                    for (i = 0; i < tb_wait_product_data.Text.Length; i++)
                    {
                        if ((i == 6) || (i == 15))
                        ...
                        else if (((tb_wait_product_data.Text[i] < '0') || (tb_wait_product_data.Text[i] > '9')) && (i != 7))
                        {
                            flag_incorrect_data = true;
                ...
                {
                    flag_incorrect_data = true;
                    richTextBox1.Text += "有資料, 但是長度錯誤, 一律清除a\n";
                    tb_wait_product_data.Text = "";
                }
        ...
        void check_export_data()
        {
            if ((flag_ok_data1 == true) && (flag_ok_data2 == true) && (flag_ok_data3 == true))
            {
                ...
                {
                    if (tb_hipot_status.Text == "OK")
                        camera_serials.Add(new string[] { tb_sn_opal_hipot.Text, "", DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") });
                    else
                        camera_serials.Add(new string[] { tb_sn_opal_hipot.Text, tb_hipot_status.Text, DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") });
                }
                else if (flag_operation_mode == MODE_RELEASE_STAGE12)
                {
                ...
                {
                    g11.Clear(BackColor);
                    g11.DrawString("存檔完成 " + csv_index11.ToString(), new Font("標楷體", 45), new SolidBrush(Color.Blue), new PointF(5, 30));
                    playSound(S_OK);

                    lb_hipot_status.Text = "狀態(1~2碼) : \t" + tb_hipot_status.Text;
                    lb_sn_opal_hipot.Text = "相機序號(9~10碼) : \t" + tb_sn_opal_hipot.Text;

                    lb_hipot_status.ForeColor = Color.Black;
                    lb_sn_opal_hipot.ForeColor = Color.Black;
        ...
        int check_network_disk()
        {
            ...
            if ((flag_operation_mode != MODE_RELEASE_STAGE1C) && (flag_operation_mode != MODE_RELEASE_STAGE2) && (flag_operation_mode != MODE_RELEASE_STAGE5)
                && (flag_operation_mode != MODE_RELEASE_STAGE6) && (flag_operation_mode != MODE_RELEASE_STAGE7) && (flag_operation_mode != MODE_RELEASE_STAGE8)
                && (flag_operation_mode != MODE_RELEASE_STAGE9) && (flag_operation_mode != MODE_RELEASE_STAGE11) && (flag_operation_mode != MODE_RELEASE_STAGE12)
        ...
        private void button49_Click(object sender, EventArgs e)
        {
            if (flag_operation_mode == MODE_RELEASE_STAGE3)
            {
                save_image_to_drive2(); //用時間檔名存檔 不檢查序號
            }
            else
            {
                save_image_to_local_drive();
        ...
        private void timer_stage6_Tick(object sender, EventArgs e)
        {
            ...
            if (tb_wait_package1_data.Text.Length > 0)
            {
                ...
                {
                    flag_incorrect_data = true;
                    richTextBox1.Text += "有資料, 但是長度錯誤, 一律清除b\n";
                    tb_wait_package1_data.Text = "";
                }
            }
        }
        ...
        private void timer_stage7_Tick(object sender, EventArgs e)
        {
            ...
            if (tb_wait_package2_data.Text.Length > 0)
            {
                ...
                {
                    flag_incorrect_data = true;
                    richTextBox1.Text += "有資料, 但是長度錯誤, 一律清除c\n";
                    tb_wait_package2_data.Text = "";
                }
        ...
        private void timer_stage8_Tick(object sender, EventArgs e)
        {
            ...
            if (tb_wait_package3_data.Text.Length > 0)
            {
                ...
                {
                    flag_incorrect_data = true;
                    richTextBox1.Text += "有資料, 但是長度錯誤, 一律清除d\n";
                    tb_wait_package3_data.Text = "";
                }
        ...
        private void timer_stage3_Tick(object sender, EventArgs e)
        {
            ...
            if (checkBox_only_picture.Checked == true)
            {
                flag_stage3_step = 3;
            }
            ...
            if (len > 0)
            {
                if ((flag_stage3_step == 0) || (flag_stage3_step == 1) || (flag_stage3_step == 3))              //等待判定等級 or 等待輸入不良原因 or 輸入相機序號
                {
                    ...
                    if (len == 1)
                    {
                        if ((tb_wait_sn_data.Text[0] == 'A') || (tb_wait_sn_data.Text[0] == 'B') || (tb_wait_sn_data.Text[0] == 'C') || (tb_wait_sn_data.Text[0] == 'E'))
                        {
                ...
                else
                {
                    ...
                    richTextBox1.Text += "有資料, 但是長度錯誤, 一律清除e\n";
                    tb_wait_sn_data.Clear();
                }
            }
        }
        ...
        private void timer_stage11_Tick(object sender, EventArgs e)
        {
            if (flag_doing_writing_data == true)
            {
                richTextBox1.Text += "正在存檔, 忽略\n";
            ...
            if (flag_ok_data1 == false)
            {
                if (tb_hipot_status.BackColor != Color.Pink)
                {
                    tb_hipot_status.BackColor = Color.Pink;
                    tb_sn_opal_hipot.BackColor = Color.White;
                }
                if ((ccc % 4) == 0)
                    lb_main_mesg11.Text = "等待輸入 狀態 \\";
                else if ((ccc % 4) == 1)
                    lb_main_mesg11.Text = "等待輸入 狀態 |";
                else if ((ccc % 4) == 2)
                    lb_main_mesg11.Text = "等待輸入 狀態 /";
            ...
            {
                if (tb_sn_opal_hipot.BackColor != Color.Pink)
                {
                    tb_hipot_status.BackColor = Color.White;
                    tb_sn_opal_hipot.BackColor = Color.Pink;
                }
                if ((ccc % 4) == 0)
                    lb_main_mesg11.Text = "等待輸入 序號 \\";
                else if ((ccc % 4) == 1)
                    lb_main_mesg11.Text = "等待輸入 序號 |";
                else if ((ccc % 4) == 2)
                    lb_main_mesg11.Text = "等待輸入 序號 /";
            ...
            if ((timer_cnt++ % 10) == 0)
            {
                ...
                if (tb_wait_hipot_data.Focused == false)
                {
                    tb_wait_hipot_data.Focus();
            ...
            else if ((len > 2) && (len <= 4))    //檢查是否換行
            {
                if ((tb_wait_hipot_data.Text[len - 2] == 0x0D) || (tb_wait_hipot_data.Text[len - 1] == 0x0A))
                {
                    tb_wait_hipot_data.Text = tb_wait_hipot_data.Text.Trim();

                    if (tb_wait_hipot_data.Text == "OK")
                    {
                        tb_hipot_status.Text = "OK";
                        flag_ok_data1 = true;
                        tb_wait_hipot_data.Clear();
                    }
                    else if (tb_wait_hipot_data.Text == "B")
                    {
                        tb_hipot_status.Text = "B : Break down (燒斷)";
                        flag_ok_data1 = true;
                        tb_wait_hipot_data.Clear();
                    }
                    else if (tb_wait_hipot_data.Text == "M")
                    {
                        tb_hipot_status.Text = "M : Max-Fail (過電流)";
                        flag_ok_data1 = true;
                        tb_wait_hipot_data.Clear();
                    }
                    else if (tb_wait_hipot_data.Text == "S")
                    {
                        tb_hipot_status.Text = "S : Short (短路)";
                        flag_ok_data1 = true;
                        tb_wait_hipot_data.Clear();
                    }
                    if (flag_ok_data1 == true)
                    {
                        g11.Clear(BackColor);
                        if (tb_hipot_status.Text == "OK")
                            g11.DrawString(tb_hipot_status.Text, new Font("標楷體", 75), new SolidBrush(Color.Blue), new PointF(130, 15));
                        else
                            g11.DrawString(tb_hipot_status.Text, new Font("標楷體", 25), new SolidBrush(Color.Blue), new PointF(5, 45));

                        lb_hipot_status.Text = "狀態(1~2碼)";
                        lb_sn_opal_hipot.Text = "相機序號(9~10碼)";
                    }
                }
                else
                {
            ...
            else if (len > 4)
            {
                if (flag_ok_data1 == true)
                {
                    int result = check_tb_sn_opal_data11();

                    if (result == S_OK)
                    {
                        tb_sn_opal_hipot.Text = tb_wait_hipot_data.Text;
                        tb_wait_hipot_data.Clear();
                        flag_ok_data2 = true;
                        flag_ok_data3 = true;
                        check_export_data();
        ...
        int check_sale3_data()
        {
            ...
            if (tb_sale3.Text.Length > 0)
            {
                ...
                */
                if ((tb_sale3.Text.Length == 39) || (tb_sale3.Text.Length == 40))
                {
                    if (tb_sale3.Text.Length == 39)         //all numbers　ex : 010000000000000017221202102012002210015
                    {
                        for (i = 0; i < tb_sale3.Text.Length; i++)
                        {
                            ...
                            {
                                flag_incorrect_data = true;
                                richTextBox1.Text += "_39資料格式不正確, 資料 : " + tb_sale3.Text + "\n";
                                tb_sale3.Clear();
                                tb_sale3.BackColor = Color.Pink;
                            }
                        }
                    ...
                    else          //len = 40  ex : 01000000000000001722120210N2012002210015
                    {
                        for (i = 0; i < tb_sale3.Text.Length; i++)
                        {
                            if (i != 26)     //position 26 contains non-numbers
                            {
                                if ((tb_sale3.Text[i] < '0') || (tb_sale3.Text[i] > '9'))
                                {
                                    flag_incorrect_data = true;
                                    richTextBox1.Text += "_40資料格式不正確, 資料 : " + tb_sale3.Text + "\n";
                                    tb_sale3.Clear();
                                    tb_sale3.BackColor = Color.Pink;
                                }
                            }
                ...
                else if (tb_sale3.Text.Length == 20)        //改成只援20碼 多一碼N
                {
                    for (i = 0; i < tb_sale3.Text.Length; i++)
                    {
                        if ((i == 6) || (i == 15))
                        ...
                        else if ((i != 7) && ((tb_sale3.Text[i] < '0') || (tb_sale3.Text[i] > '9')))
                        {
                            flag_incorrect_data = true;
                ...
                {
                    flag_incorrect_data = true;
                    richTextBox1.Text += "有資料, 但是長度錯誤, 一律清除f, 長度 : " + tb_sale3.Text.Length.ToString() + "\n";
                    richTextBox1.Text += "資料 : " + tb_sale3.Text + "\n";
                    tb_sale3.Clear();
                    tb_sale3.BackColor = Color.Pink;
                }
        ...
        void save_log_to_local_drive()
        {
            ...
            string filename = Application.StartupPath + "\\log\\iMS_Link_log." + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".txt";
        ...
        void save_image_to_drive()
        {
            ...
            if (bitmap1 != null)
            {
                ...
                if ((flag_operation_mode == MODE_RELEASE_STAGE0) || (flag_operation_mode == MODE_RELEASE_STAGE1A) || (flag_operation_mode == MODE_RELEASE_STAGE1B) || (flag_operation_mode == MODE_RELEASE_STAGE2))
                {
                    ...
                    if (flag_operation_mode == MODE_RELEASE_STAGE0)
                    {
                        richTextBox1.Text += "MODE_RELEASE_STAGE0 只存圖, 不存CSV檔, 因為存CSV檔需要相機序號\n";
                    }
                    else
                    {
                        exportCSV();
                ...
                else if (flag_operation_mode == MODE_RELEASE_STAGE3)
                {
                    if (checkBox_only_picture.Checked == true)
                    {
                        richTextBox1.Text += "第3站要求存圖不存資料, 不存CSV檔\n";
                        flag_stage3_step = 0;
                        tb_wait_sn_data.Clear();
                        lb_class.Text = "";
                    ...
                    else if ((stage3_class.Length == 1) || (stage3_class.Length == 2))
                    {
        ...
        void save_image_to_drive2() //用時間檔名存檔 不檢查序號
        {
            show_main_message1("存檔中...", S_OK, 10);
            delay(10);
            ...
            if (bitmap1 != null)
            {
                ...
                if (flag_operation_mode == MODE_RELEASE_STAGE1A)
                {
                    filename1 = "M:\\" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + "_" + "_1";
                    filename2 = Application.StartupPath + "\\picture\\" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + "_" + "_1a";
                }
                else if (flag_operation_mode == MODE_RELEASE_STAGE1B)
                {
                    filename1 = "M:\\" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + "_" + "_1";
                    filename2 = Application.StartupPath + "\\picture\\" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + "_" + "_1b";
                }
                else if ((flag_operation_mode == MODE_RELEASE_STAGE0) || (flag_operation_mode == MODE_RELEASE_STAGE2))
                {
                    filename1 = "M:\\" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + "_" + "_2";
                    filename2 = Application.StartupPath + "\\picture\\" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + "_" + "_2";
                }
                else if (flag_operation_mode == MODE_RELEASE_STAGE3)
                {
                    filename1 = "M:\\" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + "_" + "_3";
                    filename2 = Application.StartupPath + "\\picture\\" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + "_" + "_3";
                }
                else
                {
                    filename1 = "M:\\xxxx_ims_image_" + DateTime.Now.ToString("yyyyMMdd_HHmmss");
        ...
        void show_system_info()
        {
            ...
            richTextBox1.AppendText("iMS_Link登錄時間 : " + compile_time + "\n");
        ...
        void exit_program()
        {
            richTextBox1.Text += "\n\niMS_Link " + software_version + " 關閉, 時間 : " + bootup_time.ToString() + "\n\n";
            save_log_to_local_drive();
            if (flag_doing_refreshing_camera == true)
            {
                richTextBox1.Text += "正在影像重抓, 忽略\n";
...
*/
        ...
        */
        ...
        private void button82_Click(object sender, EventArgs e)
        {
            ...
            {
                serialPort1.PortName = comboBox_comport2.Text;
                serialPort1.BaudRate = int.Parse(comboBox_baud_rate2.Text);
            }
        ...
        int[] measure_brightness3(int x_st, int y_st, int ww, int hh)
        {
            if ((ww <= 0) || (hh <= 0))
            {
                richTextBox1.Text += "量測點數 ww = " + ww.ToString() + ", hh = " + hh.ToString() + " 不合法\n";
            ...
            /*
            //richTextBox1.Clear();
            richTextBox1.Text += "print histogram result\n";
            for (i = 0; i < 256; i++)
            {
                //richTextBox1.Text += i.ToString() + "\t" + Y[i].ToString();
        ...
        {
            serialPort1.PortName = comboBox_comport3.Text;
            serialPort1.BaudRate = int.Parse(comboBox_baud_rate3.Text);
        ...
        private void bt_debug2_Click(object sender, EventArgs e)
        {
            ...
            double[] sd_num = new double[] { 
                16, 19, 14, 17, 4, 4, 11, 27, 9, 8, 8, 12, 29, 28, 5, 18, 13, 7, 8, 4, 26, 25, 15, 6, 8, 12, 26, 13, 13, 27, 12, 3, 1, 8, 4, 22, 28, 23, 23, 19, 29, 17, 12, 17, 28, 19, 25, 16, 12, 7, 23, 1, 19, 24, 1, 21, 4, 8, 9, 22, 8, 27, 15, 12, 25, 6, 11, 22, 25, 13
            };
            double sd;
            sd = SD(sd_num);
            ...
            */
            ...
            richTextBox1.Text += "量測亮度\n";
            show_picture_histogram();

        }

        void test_save_picturebox()
        {
            String filename = Application.StartupPath + "\\picture\\" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".bmp";
        ...
        private void bt_debug3_Click(object sender, EventArgs e)
        {
            ...
            connect_comport_cnt_max = 0;
            richTextBox1.Clear();
            richTextBox1.Text += "測試開始\t";
            richTextBox1.Text += "時間 : " + DateTime.Now.ToString() + "\n";
            if (flag_comport_ok == true)
            {
                serialPort1.Close();
            ...
            for (i = 0; i < total_test_count; i++)
            {
                richTextBox1.Text += "\n第 " + (i + 1).ToString() + " 次\n";
                show_main_message1(i.ToString(), S_FALSE, 300);
                ...
                if (i == (total_test_count - 1))
                {
                    show_main_message1("測試完成", S_OK, 10000);
                    richTextBox1.Text += "\n測試結束\t";
                    richTextBox1.Text += "時間 : " + DateTime.Now.ToString() + "\n";
                }
                delay(100);
        ...
        void measure_picture()
        {
            ...
            */
            ...
            for (j = y_st; j < (y_st + hh); j++)
            {
                for (i = x_st; i < (x_st + ww); i++)
                {
                    ...
                    R_data[hh * (j - y_st) + (i - x_st)] = p.R;
                    G_data[hh * (j - y_st) + (i - x_st)] = p.G;
                    B_data[hh * (j - y_st) + (i - x_st)] = p.B;
            ...
            */
            ...
            for (i = 0; i < 256; i++)
            {
                ...
                curvePoints[i].Y = margin + 256 * 2 - (int)(R_count[i] * pictureBox3_ratio_y);
                //curvePoints[i].Y = 2 * i;
            }
            // Draw lines between original points to screen.
            ...
            {
                curvePoints[i].X = margin + 2 * i;
                curvePoints[i].Y = margin + 256 * 2 - (int)(G_count[i] * pictureBox3_ratio_y);
                //curvePoints[i].Y = 2 * i;
            }
            // Draw lines between original points to screen.
            ...
            {
                curvePoints[i].X = margin + 2 * i;
                curvePoints[i].Y = margin + 256 * 2 - (int)(B_count[i] * pictureBox3_ratio_y);
                //curvePoints[i].Y = 2 * i;
            }
            // Draw lines between original points to screen.
            ...
            for (i = 3; i < (256 - 3); i++)
            {
                ...
                total_G += G_count[i];
                total_B += B_count[i];
                r_array[i - 3] = (double)R_count[i];
                g_array[i - 3] = (double)G_count[i];
                b_array[i - 3] = (double)B_count[i];
                if (r_max < R_count[i])
                {
                    r_max = R_count[i];
                    r_max_index = i;
            ...
            pt1 = new Point(margin + 2 * r_max_index, margin + 256 * 2 - (int)(r_max * pictureBox3_ratio_y) - 40);
            pt2 = new Point(margin + 2 * r_max_index, margin + 256 * 2 - (int)(r_max * pictureBox3_ratio_y) + 40);
            g3.DrawLine(new Pen(Color.Blue, 2), pt1, pt2);

            pt1 = new Point(margin + 2 * g_max_index - 12, margin + 256 * 2 - (int)(g_max * pictureBox3_ratio_y));
            pt2 = new Point(margin + 2 * g_max_index + 12, margin + 256 * 2 - (int)(g_max * pictureBox3_ratio_y));
            g3.DrawLine(new Pen(Color.Red, 2), pt1, pt2);
            pt1 = new Point(margin + 2 * g_max_index, margin + 256 * 2 - (int)(g_max * pictureBox3_ratio_y) - 40);
            pt2 = new Point(margin + 2 * g_max_index, margin + 256 * 2 - (int)(g_max * pictureBox3_ratio_y) + 40);
            g3.DrawLine(new Pen(Color.Red, 2), pt1, pt2);

            pt1 = new Point(margin + 2 * b_max_index - 12, margin + 256 * 2 - (int)(b_max * pictureBox3_ratio_y));
            pt2 = new Point(margin + 2 * b_max_index + 12, margin + 256 * 2 - (int)(b_max * pictureBox3_ratio_y));
            g3.DrawLine(new Pen(Color.Green, 2), pt1, pt2);
            pt1 = new Point(margin + 2 * b_max_index, margin + 256 * 2 - (int)(b_max * pictureBox3_ratio_y) - 40);
            ...
            */
            ...
            float RR = ((float)total_R / (float)total_G) * 1024;
            float GG = ((float)total_G / (float)total_G) * 1024;
            float BB = ((float)total_B / (float)total_G) * 1024;

            g3.DrawString(RR.ToString("0.00") + " : " + GG.ToString("0.00") + " : " + BB.ToString("0.00"), new Font("標楷體", 30), new SolidBrush(Color.Green), new PointF(256 * 2 + 100 + 100, 50 + 480));
            ...
            if (flag_save_measure_color_picture == true)
            {
                flag_save_measure_color_picture = false;
                if (bmp3 != null)
                {
                    IntPtr pHdc;
                    ...
                    try
                    {
                        //bmp3.Save(@file1, ImageFormat.Jpeg);
                        bmp3.Save(filename1a, ImageFormat.Bmp);
                        //bmp3.Save(@file3, ImageFormat.Png);

                        richTextBox1.Text += "存檔成功\n";
                        //richTextBox1.Text += "已存檔 : " + file1 + "\n";
                        richTextBox1.Text += "已存檔 : " + filename1a + "\n";
                        //richTextBox1.Text += "已存檔 : " + file3 + "\n";
                        show_main_message1("存檔成功", S_OK, 30);
                    }
                    catch (Exception ex)
                    {
                        richTextBox1.Text += "xxx錯誤訊息b2 : " + ex.Message + "\n";
                ...
                16, 19, 14, 17, 4, 4, 11, 27, 9, 8, 8, 12, 29, 28, 5, 18, 13, 7, 8, 4, 26, 25, 15, 6, 8, 12, 26, 13, 13, 27, 12, 3, 1, 8, 4, 22, 28, 23, 23, 19, 29, 17, 12, 17, 28, 19, 25, 16, 12, 7, 23, 1, 19, 24, 1, 21, 4, 8, 9, 22, 8, 27, 15, 12, 25, 6, 11, 22, 25, 13
                    ...
                    sd = SD(r_array);
                    richTextBox1.Text += "SD r = " + sd.ToString() + "\n";

                    sd = SD(g_array);
                    richTextBox1.Text += "SD g = " + sd.ToString() + "\n";

                    sd = SD(b_array);
                    richTextBox1.Text += "SD b = " + sd.ToString() + "\n";


                }
                else
                {
                    richTextBox1.Text += "無圖可存c\n";
                    show_main_message1("無圖可存c", S_OK, 30);
                }
            }
        ...
        {
            richTextBox1.Text += "goto engineering mode\n";

            if (get_comport_status() == false)
                return;

            Send_IMS_Data(0xFF, 0xEE, 0xEE, 0xEE);
            show_main_message1("進入工廠模式", S_OK, 30);
            return;
        }

        private void bt_manual_mode2_Click(object sender, EventArgs e)
        {
            if (bt_manual_mode2.Text == "手動")
            {
                bt_manual_mode2.Text = "確認";
                timer_stage3.Enabled = false;
                tb_wait_sn_data.BackColor = Color.Pink;
                tb_wait_sn_data.Focus();
            }
            else
            {
                bt_manual_mode2.Text = "手動";
                timer_stage3.Enabled = true;
                tb_wait_sn_data.BackColor = Color.White;
            }
        }
        ...
        int check_camera_sensor_fail_cnt = 0;
        int check_camera_status_fail_cnt = 0;
        int get_camera_status()
        {
            ...
            int cnt = 0;
            while ((g_conn_status == CAMERA_UNKNOWN) && (cnt++ < 10))
            {
                richTextBox1.Text += "-1";
                delay(100);
            }
            ...
            {
                richTextBox1.Text += "有連接器, 有相機, 但是相機無法讀寫 " + (check_camera_sensor_fail_cnt + 1).ToString() + "\n";
                lb_write_camera_serial.Text = "相機無法讀寫 " + (check_camera_sensor_fail_cnt + 1).ToString() + "\n";
                lb_write_camera_serial2.Text = "相機無法讀寫 " + (check_camera_sensor_fail_cnt + 1).ToString() + "\n";
                lb_write_camera_serial2.BackColor = Color.Red;
                lb_write_camera_model.Text = "相機無法讀寫 " + (check_camera_sensor_fail_cnt + 1).ToString() + "\n";
                tb_info_aa1.Text = "相機無法讀寫 " + (check_camera_sensor_fail_cnt + 1).ToString() + "\n";
                tb_info_aa1.BackColor = Color.Red;
                playSound(S_FALSE);
                tb_awb_mesg.Text = "相機無法讀寫 " + (check_camera_sensor_fail_cnt + 1).ToString() + "\n";

                richTextBox1.Text += "總共檢查3次, 現在是第 " + check_camera_sensor_fail_cnt.ToString() + " 次";
                check_camera_sensor_fail_cnt++;
                if (check_camera_sensor_fail_cnt == 3)
                {
                    richTextBox1.Text += "已滿3次\n";
            ...
            else if (g_conn_status == CAMERA_OK)
            {
                ...
                lb_write_camera_serial.Text = "有連接器, 有相機";
                lb_write_camera_serial2.Text = "有連接器, 有相機";
                lb_write_camera_serial2.BackColor = Color.White;
                lb_write_camera_model.Text = "有連接器, 有相機";
                tb_info_8.Text = "有連接器, 有相機";
                tb_info_8.BackColor = Color.White;
            }
            else
            {
                richTextBox1.Text += "狀態不明b, status = " + g_conn_status.ToString() + ", 重測一次 " + (check_camera_status_fail_cnt + 1).ToString() + "\n";
                lb_write_camera_serial.Text = "狀態不明 " + g_conn_status.ToString() + ", 重測一次 " + (check_camera_status_fail_cnt + 1).ToString() + "\n";
                lb_write_camera_serial2.Text = "狀態不明 " + g_conn_status.ToString() + ", 重測一次 " + (check_camera_status_fail_cnt + 1).ToString() + "\n";
                lb_write_camera_serial2.BackColor = Color.Pink;
                lb_write_camera_model.Text = "狀態不明 " + g_conn_status.ToString() + ", 重測一次 " + (check_camera_status_fail_cnt + 1).ToString() + "\n";

                richTextBox1.Text += "總共檢查5次, 現在是第 " + check_camera_status_fail_cnt.ToString() + " 次";
                check_camera_status_fail_cnt++;
                if (check_camera_status_fail_cnt == 5)
                {
                    richTextBox1.Text += "已滿5次\n";
                    check_camera_status_fail_cnt = 0;

                    richTextBox1.Text += "狀態不明b, status = " + g_conn_status.ToString() + "\n";
                    lb_write_camera_serial.Text = "狀態不明 " + g_conn_status.ToString() + "\n";
                    lb_write_camera_serial2.Text = "狀態不明 " + g_conn_status.ToString() + "\n";
                    lb_write_camera_serial2.BackColor = Color.Pink;
                    lb_write_camera_model.Text = "狀態不明 " + g_conn_status.ToString() + "\n";
                }
                else
                {
                    get_camera_status();    //再呼叫自己一次
            ...
            if (check_camera_sensor_fail_cnt > 0)
                check_camera_sensor_fail_cnt = 0;
            if (check_camera_status_fail_cnt > 0)
                check_camera_status_fail_cnt = 0;
        ...
        private void bt_temperature_on_Click(object sender, EventArgs e)
        {
            if (flag_show_cpu_temperature == false)
            {
                ...
                this.tp_Temperature.Controls.Add(chart_temperature);
            }
        }
        ...
        private void bt_temperature_off_Click(object sender, EventArgs e)
        {
            if (flag_show_cpu_temperature == true)
            {
                ...
                lb_temperature.Text = "";
                this.tp_Temperature.Controls.Remove(chart_temperature);
            }
        }
        ...
        {
            richTextBox1.Text += "寫色彩校正預設資料進相機\n";
            ...
            richTextBox1.Text += "寫色彩校正預設資料進相機\tOK\n";
        }

        /*  debug message fo controls location
        private void button98_Click(object sender, EventArgs e)
        {
            richTextBox1.Text += "this.size = " + this.Size.ToString() + "\n";
            richTextBox1.Text += "ClientSize.size = " + this.ClientSize.ToString() + "\n";
            richTextBox1.Text += "tabControl1.size = " + this.tabControl1.Size.ToString() + "\n";
            richTextBox1.Text += "tabControl1.ClientSize = " + this.tabControl1.ClientSize.ToString() + "\n";
            richTextBox1.Text += "tabControl1.Location = " + this.tabControl1.Location.ToString() + "\n";

            richTextBox1.Text += "groupBox10.ClientSize = " + this.groupBox10.ClientSize.ToString() + "\n";
            richTextBox1.Text += "groupBox10.Location = " + this.groupBox10.Location.ToString() + "\n";
        ...
        */
        ...
        //private const int AXIS_X_MIN = -10;
        //private const int AXIS_X_MAX = 256 + 10;
        //private const int AXIS_Y_MIN = 0 - 10;
        //private const int AXIS_Y_MAX = 256 + 10;
        private const string TITLE = "";
        private const string XLABLE = "";
        private const string YLABLE = "";

        void chart2_init()
        {
            chart2.Series.Clear();  //每次使用此function前先清除圖表

            //設定Chart大小與外觀
            //全圖
            //chart2.Size = new Size(CHART_WIDTH, CHART_HEIGHT);      //改變Cahrt大小
            //chart2.Titles.Add(TITLE);                               //標題

            //X軸
            //chart2.ChartAreas[0].AxisX.Minimum = AXIS_X_MIN;        //設定X軸最小值
            //chart2.ChartAreas[0].AxisX.Maximum = AXIS_X_MAX;        //設定X軸最大值
            chart2.ChartAreas[0].AxisX.Title = XLABLE;              //設定X軸名稱
            ...
            //Y軸
            //chart2.ChartAreas[0].AxisY.Minimum = AXIS_Y_MIN;        //設定Y軸最小值
            //chart2.ChartAreas[0].AxisY.Maximum = AXIS_Y_MAX;        //設定Y軸最大值
            chart2.ChartAreas[0].AxisY.Title = YLABLE;              //設定Y軸名稱
            chart2.ChartAreas[0].AxisY.TitleForeColor = Color.Blue; //設定Y軸名稱的字體顏色
        ...
        void show_picture_brightness_histogram()
        {
            ...
            //numbers = measure_brightness3(x_st, y_st, ww, hh);
            ...
            richTextBox1.Text += "flag_right_left_cnt = " +flag_right_left_cnt.ToString() + "\n";
            */
            ...
            */
            ...
            int N = 1;
            //設定數列大小與外觀
            Series[] series = new Series[N];       //預先建立N個數組
            ...
            for (i = 0; i < N; i++)
            {
                ...
                series[i].ChartType = SeriesChartType.Line; //設定線條種類

                series[i].MarkerSize = 4;     //圖標大小
            ...
            for (i = 0; i < 256; i += 1)
            {
                series[0].Points.AddXY(i, numbers[i]);                        //將數值1新增至數組陣列0
                //series[0].Points.AddXY(i, i);                        //將數值1新增至數組陣列0
            ...
            //chart2.Series.Add(s);       //將序列1新增到chart上
        ...
        private void bt_debug4_Click(object sender, EventArgs e)
        {
