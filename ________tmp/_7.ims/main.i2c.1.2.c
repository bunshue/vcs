...
* The user needs to modify the example according to the specific devices and
* application
*
...
int IicPsMasterIntrExample(u16 DeviceId)
{
	...
	*/
	u8 data_read[3] = {3, 3, 3};
	/*
	* Initialize the IIC driver so that it's ready to use
	* Look up the configuration in the config table, then initialize it.
	*/
	Iic_ConfigPtr = XIicPs_LookupConfig(DeviceId);
	if (NULL == Iic_ConfigPtr) {
		return XST_FAILURE;
	}
	Status = XIicPs_CfgInitialize(&Iic, Iic_ConfigPtr, Iic_ConfigPtr->BaseAddress);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	xil_printf("aaa baseaddr = 0x%x\r\n", Iic_ConfigPtr->BaseAddress);
	/*
	...
	*/
	...
	//XIicPs_MasterRecv(&Iic, RecvBuffer, BufferSizes[Index],	IIC_SLAVE_ADDR);
	XIicPs_MasterRecv(&Iic, data_read, 3,	IIC_SLAVE_ADDR);
	xil_printf("data:\t");
	for(i=0;i<3;i++)
	{
		xil_printf("0x%x ", data_read[i]);
	}
	xil_printf("\n\raaa OK\r\n");
	...
	//XIicPs_MasterRecv(&Iic, RecvBuffer, BufferSizes[Index],	IIC_SLAVE_ADDR);
	XIicPs_MasterRecv(&Iic, data_read, 3,	IIC_SLAVE_ADDR + 10);
	xil_printf("data:\t");
	for(i=0;i<3;i++)
	{
		xil_printf("0x%x ", data_read[i]);
...
void Handler(void *CallBackRef, u32 Event)
{
	xil_printf("Handler %d ", TotalErrorCount);
	/*
