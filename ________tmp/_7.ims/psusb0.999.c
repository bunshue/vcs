...
int32_t ulpi_check_integrity()
{
	...
	for (i = 0; i < 2; i++) {
		ret = ulpi_WriteReg(val, ULPI_SCRATCH);	//0x16
		if (ret != 0) {
			return ret;
		}

		ret = ulpi_ReadReg(ULPI_SCRATCH);	//0x16
...
uint32_t ulpi_ReadReg(uint8_t reg)
{
...
//			}
	...
	////xil_printf("ulpi_read_data - reg: 0x%x, data: 0x%x\r\n", reg, (data >> XUSBPS_ULPIVIEW_DATRD_SHIFT) & 0xff);
	//xil_printf("i: %d\r\n", i);
