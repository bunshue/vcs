...
int periphs_gpio_init
(
		periphs_gpio_t*   p_periphs_gpio_inst,
		unsigned int ps_gpio_device_id,
		unsigned int video_lock_device_id,
		unsigned int scugic_device_id,
		unsigned int scutimer_device_id
)
{
	...
	p_periphs_gpio_inst->p_ps_gpio_inst				= &ps_gpio_inst;
	p_periphs_gpio_inst->p_lock_monitor_inst		= &lock_monitor_inst;
	p_periphs_gpio_inst->p_scugic_inst				= &scugic_inst;
	p_periphs_gpio_inst->p_scutimer_inst			= &scutimer_inst;


    init_gpio(p_periphs_gpio_inst, ps_gpio_device_id);
    init_lock_monitor(p_periphs_gpio_inst, video_lock_device_id);
    init_sugic(p_periphs_gpio_inst, scugic_device_id);
    init_scutimer(p_periphs_gpio_inst, scutimer_device_id);

	// Control Path Config
	periphs_configure_gpio_pins(p_periphs_gpio_inst); // Set up GPIO Pins
	// Connect the interrupt controller interrupt handler to the hardware interrupt handling logic in the processor.
	Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_INT,
				(Xil_ExceptionHandler)XScuGic_InterruptHandler,
				p_periphs_gpio_inst->p_scugic_inst);

	// SCUTIMER - Connect the device driver handler that will be called when an interrupt for the device occurs
	status = XScuGic_Connect(p_periphs_gpio_inst->p_scugic_inst, XPAR_SCUTIMER_INTR,
				(Xil_ExceptionHandler)scutimer_IntrHandler, p_periphs_gpio_inst->p_scutimer_inst);
	if (status != XST_SUCCESS) {
	...
	// Reset/Stop scutimer
	XScuTimer_Stop(p_periphs_gpio_inst->p_scutimer_inst);
	XScuTimer_DisableInterrupt(p_periphs_gpio_inst->p_scutimer_inst);
	XScuTimer_ClearInterruptStatus(p_periphs_gpio_inst->p_scutimer_inst);
	// Load scutimer values
	XScuTimer_LoadTimer(p_periphs_gpio_inst->p_scutimer_inst, COUNTS_PER_MILLI_SECOND);
	XScuTimer_RestartTimer(p_periphs_gpio_inst->p_scutimer_inst);
	XScuTimer_SetPrescaler(p_periphs_gpio_inst->p_scutimer_inst, 1);
	XScuTimer_EnableAutoReload(p_periphs_gpio_inst->p_scutimer_inst);
	XScuTimer_EnableInterrupt(p_periphs_gpio_inst->p_scutimer_inst);
	XScuTimer_Start(p_periphs_gpio_inst->p_scutimer_inst);
	// Enable the scugic interrupt for the scutimer.
	XScuGic_Enable(p_periphs_gpio_inst->p_scugic_inst, XPAR_SCUTIMER_INTR);

	/* Enable the interrupt for the device */
	XScuGic_Enable(p_periphs_gpio_inst->p_scugic_inst, XPAR_XUARTPS_0_INTR);
	...
	status = XScuGic_Connect(p_periphs_gpio_inst->p_scugic_inst, XPAR_XGPIOPS_0_INTR,
				(Xil_ExceptionHandler)XGpioPs_IntrHandler, p_periphs_gpio_inst->p_ps_gpio_inst);
	if (status != XST_SUCCESS) {
	...
	XGpioPs_SetIntrType(p_periphs_gpio_inst->p_ps_gpio_inst, XGPIOPS_BANK2, 0xffffffff, 0xffffffff, 0x00);

	// Set the handler for gpio interrupts. */
	XGpioPs_SetCallbackHandler(p_periphs_gpio_inst->p_ps_gpio_inst, (void *)p_periphs_gpio_inst->p_ps_gpio_inst, ps_gpio_IntrHandler);

	// Enable the GPIO interrupts of Bank 2. */
	XGpioPs_IntrEnable(p_periphs_gpio_inst->p_ps_gpio_inst, XGPIOPS_BANK2, INPUT_INTERRUPT_ALL); // TODO

	//Only used for edge sensitive Interrupts
	//XScuGic_SetPriorityTriggerType(p_periphs_gpio_inst->p_scugic_inst, XPAR_XGPIOPS_0_INTR, 0xa0, 3);

	// Enable the interrupt for the PS GPIO device.
	XScuGic_Enable(p_periphs_gpio_inst->p_scugic_inst, XPAR_XGPIOPS_0_INTR);
	...
	status = XScuGic_Connect(p_periphs_gpio_inst->p_scugic_inst, XPS_FPGA10_INT_ID,
				(Xil_ExceptionHandler)lock_monitor_IntrHandler, p_periphs_gpio_inst->p_lock_monitor_inst);
	if (status != XST_SUCCESS) {
	 ...
	 */
	XGpio_InterruptEnable(p_periphs_gpio_inst->p_lock_monitor_inst, XGPIO_IR_CH1_MASK);
	XGpio_InterruptGlobalEnable(p_periphs_gpio_inst->p_lock_monitor_inst);

	//Only used for edge sensitive Interrupts
	XScuGic_SetPriorityTriggerType(p_periphs_gpio_inst->p_scugic_inst, XPS_FPGA10_INT_ID, 0x98, 1);

	// Enable the interrupt for the lock monitor device.
	XScuGic_Enable(p_periphs_gpio_inst->p_scugic_inst, XPS_FPGA10_INT_ID);
...
void scutimer_IntrHandler(void *CallbackRef)
{
	...
	{
		if((g_ms_tick % 600) == 0)
			update_dongle_status = 1;
		else if((g_ms_tick % 600) == 200)
			update_usb_status = 1;
		//else if((g_ms_tick % 600) == 400)
			//update_usb_descriptor_status = 1;
	}
	else if(g_ms_tick > 2900)
	{
		//if((g_ms_tick % 600) == 400)
			//update_usb_descriptor_status = 1;
	}
...
void init_gpio(periphs_gpio_t* p_periphs_gpio_inst, unsigned int ps_gpio_device_id)
{
	p_periphs_gpio_inst->p_ps_gpio_inst				= &ps_gpio_inst;
	...
	status = XGpioPs_CfgInitialize(p_periphs_gpio_inst->p_ps_gpio_inst, p_ps_gpio_cfg, p_ps_gpio_cfg->BaseAddr);
	if (status != XST_SUCCESS)
	{
		xil_printf("ERROR! Failed to initialize the PS GPIO.\n\r");
		return PERIPHS_ERROR_UNKNOWN;
...
void init_lock_monitor(periphs_gpio_t* p_periphs_gpio_inst, unsigned int video_lock_device_id)
{
	p_periphs_gpio_inst->p_lock_monitor_inst			= &lock_monitor_inst;
	...
	status = XGpio_Initialize(p_periphs_gpio_inst->p_lock_monitor_inst, video_lock_device_id);
	if (status != XST_SUCCESS)
	{
		xil_printf("ERROR! Failed to initialize the video lock monitor.\n\r");
		return PERIPHS_ERROR_UNKNOWN;
...
void init_sugic(periphs_gpio_t* p_periphs_gpio_inst, unsigned int scugic_device_id)
{
	p_periphs_gpio_inst->p_scugic_inst				= &scugic_inst;
	int              status         = 0;
	XScuGic_Config*  p_scugic_cfg;
	...
	status = XScuGic_CfgInitialize(p_periphs_gpio_inst->p_scugic_inst, p_scugic_cfg, p_scugic_cfg->CpuBaseAddress);
	if (status != XST_SUCCESS) {
		xil_printf("ERROR! Failed to initialize scugic.\n\r");
		return XST_FAILURE;
	}
...
void init_scutimer(periphs_gpio_t* p_periphs_gpio_inst, unsigned int scutimer_device_id)
{
	p_periphs_gpio_inst->p_scutimer_inst				= &scutimer_inst;
	...
	status = XScuTimer_CfgInitialize(p_periphs_gpio_inst->p_scutimer_inst, p_scutimer_cfg, p_scutimer_cfg->BaseAddr);
	if (status != XST_SUCCESS) {
		xil_printf("ERROR! Failed to initialize scutimer.\n\r");
		return XST_FAILURE;
	}
