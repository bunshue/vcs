...
)
{
	...
	xil_printf("david0712: %s:%s(%d) ST aaa\r\n",__FILE__,__func__,__LINE__);
	...
	// Select SI570
	wr_buf = 0x01;
	xil_printf("david0712: %s:%s(%d) ST bbb\r\n",__FILE__,__func__,__LINE__);
...
unsigned char si570_i2c_read_reg
(
	XIicPs*        p_si570_iic_inst,
	unsigned char  reg_addr
)
{
	...
	xil_printf("david0712: %s:%s(%d) ST si570_i2c_read_reg addr=%d\r\n",__FILE__,__func__,__LINE__,reg_addr);
	...
	xil_printf("david0712: %s:%s(%d) SP and return 0x%x\r\n",__FILE__,__func__,__LINE__,reg[0]);
	return reg[0];
}
