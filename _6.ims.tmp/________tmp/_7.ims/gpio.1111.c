...
void ps_gpio_IntrHandler(void *CallBackRef, u32 Bank, u32 Status)
{
	...
	{
		//xil_printf("ps_gpio_IntrHandler xxxxxxxxxxx\r\n");
		return;
	}
	//									2
	DataRead = XGpioPs_Read(Gpio, XGPIOPS_BANK2);	//read bank 2 data, seems useless

	//xil_printf("ps_gpio_IntrHandler, Bank: %x\r\n", Bank);
	//xil_printf("ps_gpio_IntrHandler, DataRead = 0x%x = %u, Status = 0x%x = %d\r\n", DataRead, DataRead, Status, Status);
	//xil_printf("Status = %d, SW_SMART = %d, g_procedure_started = %d\r\n", Status, SW_SMART, g_procedure_started);
	xil_printf("\n\r\n\rps_gpio_IntrHandler ST DataRead = 0x%x\tStatus = 0x%x\t====>\t", DataRead, Status);
    switch (Status)
	{
		case SW_SMART:	xil_printf("Smart mode\n\r");break;
		case SW_START:	xil_printf("Start\n\r");break;
		case SW_METER:	xil_printf("Metering\n\r");break;
		case SW_DARKEN:	xil_printf("Darken\n\r");break;
		case SW_BRIGHT:	xil_printf("Brighten\n\r");break;
		default:		xil_printf("xxxxxxxx status = 0x%x\n\r", Status);break;
	}
...
//		g_exposure = 3;
		...
		{
			xil_printf("david0629: %s:%s(%d) ST press-key no work, procedure_started = %d\r\n",__FILE__,__func__,__LINE__, g_procedure_started);
		}
	}
	//					0x1000=4096
...
//	}
	...
	xil_printf("david0629: %s:%s(%d) ST g_updated = 1\r\n",__FILE__,__func__,__LINE__);
