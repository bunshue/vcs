...
unsigned int psuart0_dongle_ping_old(void)
{
	uint32_t Index;
...
unsigned int psuart0_dongle_ping(void)
{
    ...
    //xil_printf("video_status = %d\r\n", video_status);

	if(flag_camera_video_status == 1)
	{
	...
	while (1) {
		 ...
		 * as the return value since we are using it in interrupt mode.
		 */
		XUartPs_Send(periphs_inst.p_psuart0_inst, SendBuffer, 4);
		...
		if (TotalSentCount == 4 && TotalReceivedCount == 0)
		{
			//xil_printf("no dongle\r\n");
			return DONGLE_NONE; // No dongle or camera
		}
...
unsigned int psuart0_dongle_ping2(void)
{
	uint32_t Index;
...
void psuart0_exposure(unsigned int exposure)
{
	...
	//int flag_use_user_brightness;
	//int default_user_brightness;

	if(flag_use_user_brightness == TRUE)
	{
		if(default_user_brightness < 50)
			default_user_brightness = 50;

		switch(exposure)
		{
			case 1:
				WPT = default_user_brightness - 40;
				break;
			case 2:
				WPT = default_user_brightness - 20;
				break;
			case 3:
				WPT = default_user_brightness;
				break;
			case 4:
				WPT = default_user_brightness + 20;
				break;
	...
	xil_printf("psuart0_exposure setup %d, WPT = 0x%02x = %d, BPT = 0x%02x = %d\n\r", exposure, WPT, WPT, BPT, BPT);
...
//	}
	...
	setup_panel_brightness_led(g_exposure);
	return;
}
...
void PROG_1PAGE()
{
	...
	else if((page_data[14] == 0x88) && (page_data[15] == 0x88))
	{
		xil_printf("write user serial data 8\n\r");
		for(i = 0; i < 16; i++)
		{
			SendBuffer[8 + i] = user_camera_data8[i];
		}
	}
	else if((page_data[14] == 0x99) && (page_data[15] == 0x99))
	{
		xil_printf("write user serial data 9\n\r");
		for(i = 0; i < 16; i++)
		{
			SendBuffer[8 + i] = user_camera_data9[i];
		}
	}
	else if((page_data[12] == 0xE0) && (page_data[13] == 0xE0) && (page_data[14] == 0xE0) && (page_data[15] == 0xE0))
	{
		xil_printf("write camera model\n\r");
		for(i = 0; i < 16; i++)
		{
			SendBuffer[8 + i] = user_camera_data[i];
		}
	}
	else if((page_data[12] == 0xE2) && (page_data[13] == 0xE2) && (page_data[14] == 0xE2) && (page_data[15] == 0xE2))
	{
		xil_printf("write camera awb data aaaaaaaaa\n\r");
		for(i = 0; i < 16; i++)
		{
			SendBuffer[8 + i] = user_camera_data[i];
		}
	...
	else
	{
		...
		*/
		...
		SendBuffer[8 + 8] = 0x00;
		SendBuffer[8 + 9] = 0x00;
		SendBuffer[8 + 10] = 0x00;
		SendBuffer[8 + 11] = 0x00;
		SendBuffer[8 + 12] = 0x00;
		SendBuffer[8 + 13] = 0x00;
		SendBuffer[8 + 14] = 0x00;
		SendBuffer[8 + 15] = 0x00;
	}
