...
/* This is the routine that generates the image to be displayed. */
{
   ...
   glEnable (GL_MAP1_VERTEX_3);
    ...
    glNewList (1, GL_COMPILE);
        ...
        glPointSize (4.0);
        glBegin (GL_POINTS);
            for (i=0; i<number_of_points; i++) glVertex3dv (&points[i][0]);
       glEnd ();
    glEndList ();
    ...
    glNewList (2, GL_COMPILE);
        ...
        for (j=0; j<number_of_points; j+=3)
        {
            ...
            glBegin (GL_LINE_STRIP);
                for (i=0; i<=STEPS; i++) glEvalCoord1d (i);
            glEnd ();
        }
    glEndList ();
}
   ...
   size has changed. */
{
    ...
    glLoadIdentity ();
    gluOrtho2D (minx-0.05*xrange, maxx+0.05*xrange, miny-0.05*yrange,
        maxy+0.05*yrange);
}
...
/* This function gets the input data for the program to process. */
{
   ...
   {
      if (points[number_of_points][0] < minx) minx = points[number_of_points][0];
      if (points[number_of_points][0] > maxx) maxx = points[number_of_points][0];
      if (points[number_of_points][1] < miny) miny = points[number_of_points][1];
      if (points[number_of_points][1] > maxy) maxy = points[number_of_points][1];
   ...
   points[number_of_points+1][0] = points[number_of_points][0] =
       points[number_of_points-1][0];
   points[number_of_points+1][1] = points[number_of_points][1] =
       points[number_of_points-1][1];
