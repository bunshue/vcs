...
/*
統計顏色這一塊，其實我一直在尋找一種即不用占很大內存，速度又快的算法，但是一直沒有想到好辦法。
上面的代碼中是分配了64MB的內存來索引計數的，雖然對於很小的圖像也需要這麼大的內存占用量，
但是我經過對比發現，比用Dictionary之類的基於字典的統計方法還是要快很多的。

關於排序，我一直認為自己能寫出比系統更快的算法，但是最終我還是選擇了如上代碼中的簡便方式。
在對Amount進行排序的同時，Color的值也跟著隨動了。

在這種占用比較大內存的代碼中，我認為應該立即調用GC.Collect()釋放掉內存。

關於Delta的取值，似乎不太好確定，這個只能說試驗確定吧，一般取16-32之間比較合理。

兩個參考鏈接處也有一些比較好的算法的，不過裡面的代碼是PHP的，改寫成C#的應該說還是有一定的難度的。

從個人的理解來看，我覺得這種顏色主成分分析 還可以利用 類似於彩色轉索引時
找最佳索引表時用的八叉樹算法；也可以用FCM或者KMEANS之類的聚類算法來實現。
*/
