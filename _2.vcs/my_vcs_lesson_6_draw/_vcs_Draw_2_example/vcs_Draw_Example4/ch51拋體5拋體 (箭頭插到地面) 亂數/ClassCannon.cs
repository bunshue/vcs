...
namespace WindowsApplication1
{
    class ClassCannon
    {
        ...
        int xL, yL;  // Last x , Last y  上一次的位置
        public bool alive = true; // 是否還需要更新  (箭頭插到地面)
        double X1, Y1, X2, Y2;  // 箭頭的起點和終點
        double dist; // 現在的位置 和 上一次的位置 的距離
       ...
       public void Draw(Graphics G)
       {
           if (alive)
           {
               ...
               x = x0 + (int)(v0 * Math.Cos(theta+rd.NextDouble() * 0.001) * t) ;
               y = y0 + (int)(v0 * Math.Sin(theta + rd.NextDouble() * 0.001) * t + g * t * t / 2); // -g * t * t / 2
               ...
               dist = Math.Sqrt((x - xL) * (x - xL) + (y - yL) * (y - yL));
