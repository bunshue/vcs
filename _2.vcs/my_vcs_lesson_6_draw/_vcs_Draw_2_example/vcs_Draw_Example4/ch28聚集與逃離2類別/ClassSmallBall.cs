...
namespace WindowsApplication11
{
    ...
    {
        PointF pos = new PointF(); // 小圓點的座標
        Random rd;  // 亂數
        int ClientWidth, ClientHeight; // 活動區域的寬高

        public ClassSmallBall(int ClientWidth, int ClientHeight, int seed)
        {
            rd = new Random(seed); // 亂數表打散
            pos.X = rd.Next(ClientWidth); // 得到小圓點的座標
            pos.Y = rd.Next(ClientHeight);
            this.ClientWidth = ClientWidth; // 紀錄活動區域的寬高
        ...
        {
            G.FillEllipse(Brushes.Silver, pos.X - 5, pos.Y - 5, 10, 10);
        }
    }
}
