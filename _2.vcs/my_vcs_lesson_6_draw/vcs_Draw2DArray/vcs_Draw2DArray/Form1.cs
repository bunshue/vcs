            ...
            */
            ...
            for (j = 0; j < M; j++)
            {
                for (i = 0; i < N; i++)
                {
                    ...
                    if (max < brightness[j, i])
                        max = brightness[j, i];
                    else if (min > brightness[j, i])
                        min = brightness[j, i];
                    total += brightness[j, i];
                }
                //richTextBox1.Text += "\n";
            }
            ...
            for (j = 0; j < M; j++)
            {
                for (i = 0; i < N; i++)
                {
                    rms += (brightness[j, i] - average) * (brightness[j, i] - average);
                }
            }
            rms /= M * N;
            ...
            for (yy = 0; yy < height; yy++)
            {
                ...
                {
                    //設置像素的ＲＧＢ顏色值
                    rr = (byte)((brightness[yy / dd, xx / dd] - min) * 255 / diff);
                    gg = (byte)((brightness[yy / dd, xx / dd] - min) * 255 / diff);
                    bb = (byte)((brightness[yy / dd, xx / dd] - min) * 255 / diff);
                    bitmap1.SetPixel(xx, yy, Color.FromArgb(aa, rr, gg, bb));
                }
            }
            pictureBox1.Image = bitmap1;
