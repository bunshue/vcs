...
namespace howto_401k_graph
{
    public partial class Form1 : Form
    {
        ...
        {
            decimal annual_contribution = decimal.Parse(txtAnnualContribution.Text, NumberStyles.Any);

            richTextBox1.Text += "annual_contribution = " + annual_contribution.ToString() + "\n";

            decimal tax_rate = decimal.Parse(txtTaxRate.Text.Replace("%", ""), NumberStyles.Any) / 100;
            ...
            decimal interest_rate = decimal.Parse(txtInterestRate.Text.Replace("%", ""), NumberStyles.Any) / 100;
            ...
            for (int year = 0; year < num_years; year++)
            {
                ...
                balance_roth += balance_roth * interest_rate + annual_contribution * (1 - tax_rate);
            ...
            decimal penalty = decimal.Parse(txtPenalty.Text.Replace("%", ""), NumberStyles.Any) / 100;

            richTextBox1.Text += "penalty = " + penalty.ToString() + "\n";
            if (penalty >= 1) penalty /= 100;
            richTextBox1.Text += "penalty = " + penalty.ToString() + "\n";

            decimal tax_at_end = decimal.Parse(txtTaxAtEnd.Text.Replace("%", ""), NumberStyles.Any) / 100;
            ...
            using (Graphics gr = Graphics.FromImage(bm))
            {
                ...
                //gr.TranslateTransform(0, pt_401k[2 * num_years - 1].Y, MatrixOrder.Append);
                gr.TranslateTransform(0, picGraph.ClientSize.Height - 3, MatrixOrder.Append);
                ...
                gr.ResetTransform();
                gr.FillEllipse(Brushes.Red, transformed_points[0].X - 2, transformed_points[0].Y - 2, 5, 5);
                gr.FillEllipse(Brushes.Blue, transformed_points[1].X - 2, transformed_points[1].Y - 2, 5, 5);
                gr.FillEllipse(Brushes.Green, transformed_points[2].X - 2, transformed_points[2].Y - 2, 5, 5);
            }
