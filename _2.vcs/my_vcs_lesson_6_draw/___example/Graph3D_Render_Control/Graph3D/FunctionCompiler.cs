...
namespace Plot3D
{
    ...
    {
        static String EVAL_CLASS = "using System;\n"
                                 + "using {1};\n"
                                 + "public class Eval\n"
                                 + "{{\n"
        ...
        /// </code>
        /// </example>
        public static CompiledFunction Compile(string functionBody)
        {
            functionBody = functionBody.Trim().ToLower();
            if (functionBody.Contains(";"))
                throw new FunctionNotCompiledException("Function string cannot contain semicolon");
            ...
            string str = string.Format(EVAL_CLASS, functionBody, typeof(CompiledFunction).Namespace);
