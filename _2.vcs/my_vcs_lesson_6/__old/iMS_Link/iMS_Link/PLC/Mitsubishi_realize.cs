...
namespace iMS_Link.PLC_Communication
{
    /// <summary>
    ...
    class Mitsubishi_realize : IPLC_interface
    {
        ...
        /// <param name="Name"></param>
        /// <param name="id"></param>
        /// <param name="PLC_STATE"></param>
        /// <returns></returns>
        List<bool> IPLC_interface.PLC_write_M_bit(string Name, string id, PLC_STATE plc_state)//寫入PLC 位狀態 --D_bit這類需要自己在表流獲取當前位狀態--M這類不需要
        {
            string result = "FALSE";//定義獲取數據變量
            lock (this)
            {
                try
                {
                    ...
                    //bool write_data = (plc_state == PLC_STATE.ON) ? true : false;
