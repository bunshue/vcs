...
namespace vcs_WebCam
{
    public partial class Form1 : Form
    {
        ...
        bool flag_cellphone_mode = false;
        bool flag_cellphone_rotate_mode = false;
        int flag_cellphone_direction = 0;   //0: 水平模式, 1: 轉90度, 2: 轉180度, 3: 轉270度
        ...
        void show_item_location()
        {
            ...
            bt_cellphone.Location = new Point(x_st + dx * 2, y_st + dy * 2);
            bt_help.Location = new Point(x_st + dx * 3, y_st + dy * 2);
            bt_open_folder.Location = new Point(x_st + dx * 4 - 5, y_st + dy * 2 + 30);
            ...
            bt_cellphone.Enabled = true;
            flag_cellphone_mode = false;
        ...
        private void Cam_NewFrame(object sender, NewFrameEventArgs eventArgs)
        {
            ...
            if ((cb_show_time.Checked == false) && (cb_image_processing.Checked == false))      //直接顯示圖片
            {
                ...
                if (flag_cellphone_mode == true)
                {
                    //int flag_cellphone_direction = 0;   //0: 水平模式, 1: 轉90度, 2: 轉180度, 3: 轉270度
                    if ((flag_cellphone_direction == 1) || (flag_cellphone_direction == 3))
                    {
                        if (flag_cellphone_rotate_mode == true)
                        {
                            if (webcam_w > webcam_h)
                            {
                                RectangleF rect = new RectangleF(0, webcam_w / 3, webcam_h, webcam_w / 3);
                                bm = bm.Clone(rect, PixelFormat.Format32bppArgb);
                            }
                        }
                    }
                }
        ...
        void pictureBox1_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)   //根據e.KeyCode分別執行
            {
                ...
                case Keys.S:
                    ...
                    break;
                case Keys.R:
                    flag_cellphone_rotate_mode = false;
                    flag_cellphone_direction++;
                    if (flag_cellphone_direction > 3)
                        flag_cellphone_direction = 0;
                    //flag_cellphone_direction   //0: 水平模式, 1: 轉90度, 2: 轉180度, 3: 轉270度
                    if (flag_cellphone_direction == 0)
                        rotate_flip_type = RotateFlipType.RotateNoneFlipNone;
                    else if (flag_cellphone_direction == 1)
                        rotate_flip_type = RotateFlipType.Rotate90FlipNone;
                    else if (flag_cellphone_direction == 2)
                        rotate_flip_type = RotateFlipType.Rotate180FlipNone;
                    else if (flag_cellphone_direction == 3)
                        rotate_flip_type = RotateFlipType.Rotate270FlipNone;

                    break;
                case Keys.T:
                    flag_cellphone_rotate_mode = !flag_cellphone_rotate_mode;
                    richTextBox1.Text += "旋轉方向 : " + rotate_flip_type.ToString() + "\n";
                    richTextBox1.Text += "rotate_mode : " + flag_cellphone_rotate_mode + "\n";
        ...
        private void bt_cellphone_Click(object sender, EventArgs e)
        {
            flag_cellphone_mode = true;
