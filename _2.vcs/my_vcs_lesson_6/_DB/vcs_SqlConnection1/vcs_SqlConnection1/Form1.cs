...
namespace vcs_SqlConnection1
{
    public partial class Form1 : Form
    {
        ...
        void sql_read_database(string db_filename, string sqlstr, DataGridView dgv)
        {
            ...
            {
                lb_dgv2.Text = "";
                lb_dgv2b.Text = db_filename;
            }
            else if (dgv == dataGridView3)
            {
                lb_dgv3.Text = "";
                lb_dgv3b.Text = db_filename;
            }
            else if (dgv == dataGridView4)
            {
                lb_dgv4.Text = "";
                lb_dgv4b.Text = db_filename;
            }
        }
        ...
        private void button0_Click(object sender, EventArgs e)
        {
            ...
            string strid = string.Empty;  // 員工編號
            ...
            */
            ...
            // 查詢字串, 將符合條件的資料取資料總和
            sqlstr = "SELECT SUM(Year_M1+Year_M2+Year_M3+Year_M4+Year_M5+Year_M6+Year_M7+Year_M8+Year_M9+Year_M10+Year_M11+Year_M12) AS 年總和 FROM tb_Stat WHERE ShowYear=" + query_year + "";

            sql_read_database(db_filename, sqlstr, dataGridView2);
            lb_dgv2.Text = "某年資料 12個月數目總和";
            ...
            {
                // 查詢字串
                sqlstr = "SELECT power FROM tb_power WHERE name='" + name + "'";
                SqlCommand cmd = new SqlCommand(sqlstr, cn);//連接SQL語句與數擾庫的連接
                cmd.Connection = cn;
            ...
            db_filename = "db_10_Data.MDF";
            // 查詢字串
            sqlstr = "SELECT * FROM 工資表";
            sql_read_database(db_filename, sqlstr, dataGridView1);

            richTextBox1.Text += "------------------------------\n";  // 30個

            // 查詢字串
            sqlstr = "SELECT * FROM 工資表 ORDER BY 所屬部門";
...
sqlstr = "SELECT 產品編號,產品名稱 FROM tb_WidgetApply";//定義一個數據庫查詢字符串
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            */
            ...
            string strTableName = "table_name"; // animals1_table
            // 查詢字串
            sqlstr = "SELECT name 字段名, xusertype 類型編號, length 長度 into hy_Linshibiao FROM syscolumns WHERE id=object_id('" + strTableName + "') ";

            sqlstr = "SELECT name 字段名, xusertype 類型編號, length 長度 into animals1_table FROM syscolumns WHERE id=object_id('animals1_table')";
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection cn = new SqlConnection(cnstr))  // 建立資料庫連接對象cn
            {
                try
                {
                    ...
                    // 查詢字串, 新增資料, 使用 Parameters
                    sqlstr = "INSERT INTO 員工訊息 VALUES (@員工編號,@姓名,@照片,@性別,@出生日期,@籍貫,@工齡,@電話,@部門名稱)";
                    SqlCommand cmd = new SqlCommand(sqlstr, cn);
            ...
            using (SqlConnection cn = new SqlConnection(cnstr))  // 建立資料庫連接對象cn
            {
                ...
                using (SqlCommand cmd = new SqlCommand(sqlstr, cn))
                {
                    ...
                    while (dr.Read())  // 讀取一筆資料到dr
                    {
                        //richTextBox1.Text += "第0項 : " + dr.GetString(0) + "\n";
                        //richTextBox1.Text += "第1項 : " + dr.GetString(1) + "\n";
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            //新增
            string book_id = "IMS0311";  // 書號 為 PK, 不可重複
            string book_name = "膠囊內視鏡";
            int price = 12345;
            int quantity = 20;
            ...
            using (SqlConnection cn = new SqlConnection(cnstr))  // 建立資料庫連接對象cn
            {
                ...
                cmd.Parameters["@BookName"].Value = book_name;
            ...
            dataGridView2.Rows[12].DefaultCellStyle.BackColor = Color.LightPink;  // 更換背景色, 列
            dataGridView2.Rows[12].Cells[0].Style.BackColor = Color.Red;  // 改變背景色

            richTextBox1.Text += "------------------------------\n";  // 30個

            //更新
            book_id = "IMS0311";//以書號為準
            book_name = "ims EGDaxx";
            price = 1231;
            quantity = 181;
            ...
            dataGridView3.Rows[12].DefaultCellStyle.BackColor = Color.LightPink;  // 更換背景色, 列
            dataGridView3.Rows[12].Cells[0].Style.BackColor = Color.Red;  // 改變背景色
        ...
        {
            //插入多筆紀錄, 將A表單的資料增加到B表單, 同一資料庫
            // A 表單 tb_Student
            // B 表單 tb_Student_Copy
        ...
        private void button12_Click(object sender, EventArgs e)
        {
            ...
            cnstr = string.Format(db_cnstr, db_filename);
            // 查詢字串
            sqlstr = "SELECT * FROM tb_stu WHERE 學生編號 LIKE '"
        ...
        private void button14_Click(object sender, EventArgs e)
        {
            ...
            cnstr = string.Format(db_cnstr, db_filename);
            // 查詢字串
            sqlstr = "SELECT DISTINCT (SELECT AVG(成績) FROM tb_Stud  WHERE 課程='" + course + "') AS 平均成績 FROM tb_Stud";
        ...
        private void button16_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection cn = new SqlConnection(cnstr))  // 建立資料庫連接對象cn
            {
                SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM 明細工資表 WHERE (基本工資 IN (SELECT 基本工資 FROM 明細工資表 WHERE 基本工資 > " + money_low.ToString() + " AND 基本工資 < " + money_high.ToString() + "))", cn);
                DataSet ds = new DataSet();  // 建立數據集ds, 準備給da用來填充數據(Table格式)
                da.Fill(ds, "table");
        ...
        private void button17_Click(object sender, EventArgs e)
        {
            ...
            sqlstr = "DROP TABLE animals1_table";
            // 不執行 sql_write_database(db_filename, sqlstr);  // 執行SQL命令
            ...
            // 查詢字串
            string cname = "AAA";
            sqlstr = "DELETE FROM animals1_table WHERE 中文名 = '" + cname + "'";
            // 不執行 sql_write_database(db_filename, sqlstr);  // 執行SQL命令
            ...
            using (SqlConnection cn = new SqlConnection(cnstr))  // 建立資料庫連接對象cn
            {
                cn.Open();
                SqlCommand cmd = new SqlCommand(sqlstr, cn);
                cmd.Parameters.Add(new SqlParameter("@cname", SqlDbType.NVarChar));
                cmd.Parameters["@cname"].Value = cname;
                // 不執行 cmd.ExecuteNonQuery();  // 執行SQL命令
            }
        }
        ...
        private void button19_Click(object sender, EventArgs e)
        {
            ...
            */
            ...
            //TBD
            // 查詢字串, 一次搜尋多個
            sqlstr = "SELECT * FROM animals1_table WHERE 中文名 IN(N'丹尼狗', N'巧虎')";
            sql_read_database(db_filename, sqlstr, dataGridView2);
            lb_dgv2.Text = "一次搜尋多個";
            ...
            // 查詢字串, 取得欄資料的最大值和最小值
            sqlstr = "SELECT MAX(體重), MIN(體重) FROM animals1_table";
            sql_read_database(db_filename, sqlstr, dataGridView4);
            */
            ...
            sqlstr = "SELECT COUNT(體重) FROM animals1_table";
            obj = sql_get_database_data(db_filename, sqlstr);
            richTextBox1.Text += "共 " + obj.ToString() + " 筆記錄b\n";

            // 查詢字串, COUNT(DISTINCT 列名) 統計去重後的數量
            sqlstr = "SELECT COUNT(DISTINCT 體重) FROM animals1_table";
            obj = sql_get_database_data(db_filename, sqlstr);
        ...
        private void button22_Click(object sender, EventArgs e)
        {
            ...
            sqlstr = "INSERT INTO animals1_table (編號, 英文名, 中文名, 體重) VALUES (95, N'aaa', N'AAA', 20)";
            sqlstr = "INSERT INTO animals1_table (編號, 英文名, 中文名, 體重) VALUES (98, N'bbb', N'BBB', 35)";
            sqlstrs.Add(sqlstr);
            sqlstrs.Add(sqlstr);
            sqlstrs.Add(sqlstr);
            sqlstrs.Add(sqlstr);
        ...
        private void button28_Click(object sender, EventArgs e)
        {
            ...
            sqlstr = "DELETE FROM 員工訊息 WHERE 員工編號=N'" + "P1008" + "'";

            // 用 LIKE 看看
            sqlstr = "DELETE FROM 員工訊息 WHERE 員工編號 LIKE '%" + "A12345" + "%'";
...
/*
// 查詢字串, 使用別名 AS
sqlstr =  "SELECT au_id AS 使用者編號, au_lname AS 使用者名,phone AS 聯繫電話 FROM authors";

"SELECT " + comboBox1.Text + "," + comboBox1.Text + " AS " + textBox1.Text.Trim() + " FROM tb_02";
"SELECT * FROM tb_08 WHERE " + comboBox1.Text + " IS null OR " + comboBox1.Text + "=''", cn);

//sqlstr =  "UPDATE 系統管理員表 SET 使用者名稱='" + textBox1.Text + "',密碼='" + textBox2.Text + "'";
