...
namespace vcs_ox_game
{
    public partial class Form1 : Form
    {
        ...
        private void Check()
        {
            richTextBox1.Text += "Check() btn.GetLength(0) = " + btn.GetLength(0).ToString() + " btn.GetLength(1) = " + btn.GetLength(1).ToString() + "\n";
            for (int x = 0; x < btn.GetLength(0); x++)
            {
                richTextBox1.Text += "a";
                if (btn[x, 0].Text == btn[x, 1].Text && btn[x, 1].Text == btn[x, 2].Text && btn[x, 0].Text != "" && btn[x, 1].Text != "" && btn[x, 2].Text != "")
                {
                    richTextBox1.Text += "Check() 1\n";
