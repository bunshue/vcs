...
namespace 保齡球網路遊戲設計_server
{
    public partial class Form1 : Form
    {
        ...
        private void ClientService() //這個副程式，乃是無限迴圈中偵測是否有Client傳送訊息
        {
            ...
            int ii, name = 0, score = 0, count_temp = 0, update = 0, score1 = 0;
            string strAll = "";
            Socket socketTemp;
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    ii = sock.ReceiveFrom(byteReceieve, 0, byteReceieve.Length, SocketFlags.None, ref oldEP);//使用指定的 SocketFlags，//接收指定位元組數目的資料至資料緩衝區的指定位置，並儲存端點。 
                    ...
                    if (disconnect == 1)
                    {
                        if ((strAll != "disconnect") && (strAll != ""))
                        {
                            ...
                            if (c_conn <= 3)
                            {
                                byteSend = Encoding.Unicode.GetBytes("AAA".ToCharArray());
                    ...
                    else  // 有人進來時
                    {
                        ...
                        else if ((name == 1))
                        {
                            ...
                            Show_label2();
                            Show_label3(strAll);
                            foreach (DictionaryEntry de in htname)   //把上線者的名稱暫存在temp裡面
                            {
                            ...
                            if ((c_conn >= 4))
                            {
                                byteSend = Encoding.Unicode.GetBytes("four".ToCharArray());
                                foreach (DictionaryEntry dc in ht)
                                {
