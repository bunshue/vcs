...
namespace WindowsFormsApplication1
{
    class G2D_ImageRotateBias
    {
        ...
        public G2D_ImageRotateBias(Bitmap bitmap, Point pos, Point pivot)
        {
            this.bitmap = bitmap;
            this.pos = pos;
            ...
            disInBitmap = Math.Sqrt(bitmap.Width * bitmap.Width / 4 + bitmap.Height * bitmap.Height / 4);
        ...
        // 點 p 是否落在 目前圖形 四個角內
        public bool isPointInBitmap2(Point p)
        {
            Point[] original = new Point[4];
            original[0] = new Point(0, 0);
            original[1] = new Point(bitmap.Width, 0);
            original[2] = new Point(0, bitmap.Height);
        ...
        public void MouseDown(Point e)
        {
            ...
            else if (isPointInBitmap(e)) // 滑鼠游標在 圖形上 => 要旋轉圖形
            {
                Inertia = 0;  // 取消 旋轉慣性 
                timer1.Enabled = false;
