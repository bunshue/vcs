    ...
    public partial class Form1 : Form
    {
        ...
        void Go()
        {
            ...
            while (keepSearching)
            {
                ...
                {
                    if (cList[k].Neighbor[i] == -1)
                    {
                        continue;  // no this Neighbor
                    }
                    else if (wallList.IndexOf(cList[k].Neighbor[i]) != -1)
                    {
                        continue; // this Neighbor is a wall
                    }
                    else if (closedList.IndexOf(cList[k].Neighbor[i]) != -1)
                    {
                        continue; // this Neighbor is in closedList
                    }
                    else if (openList.IndexOf(cList[k].Neighbor[i]) == -1)  // this Neighbor is not in openList
                    {
                        ...
                        cList[cList[k].Neighbor[i]].H = 10 * (Math.Abs(cList[Target].row - cList[cList[k].Neighbor[i]].row) + Math.Abs(cList[Target].column - cList[cList[k].Neighbor[i]].column));
                    }
