...
WHERE id=@id",
            ...
            da.UpdateCommand = cmd;//设置UpdateCommand属性
            da.Update(G_st.Tables[0]);//更新数据库中数据
        ...
        private void GetMessage()
        {
            ...
            string cnstr = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=D:\db_TomeTwo.mdf;Integrated Security=True;Connect Timeout=30";
            // 查詢字串
            string sqlstr = string.Format("SELECT * FROM tb_Student_Copy");
            //创建数据适配器
            SqlDataAdapter da = new SqlDataAdapter(sqlstr, cnstr);
            da.Fill(G_st.Tables[0]);//填充数据表
        }
