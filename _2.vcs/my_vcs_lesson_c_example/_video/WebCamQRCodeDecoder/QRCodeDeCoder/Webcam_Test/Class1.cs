...
namespace Microshaoft.Text
{
        ...
        internal static string[] nicename = new string[]
   {
    ...
    , "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER"
        ...
        /// <code>
        ///  IdentifyEncoding ide = new IdentifyEncoding();
        ///  Response.Write(ide.GetEncodingName(IdentifyEncoding.ToSByteArray(System.Text.Encoding.GetEncoding("gb2312").GetBytes("Lion互動網絡(www.lionsky.net)")))); 
        /// </code>
        /// </example>
        ...
        /// <summary>
        /// 將此實例中的指定 <see cref="sbyte"/> 字符數組轉換到 <see cref="byte"/> 字符數組。
        /// </summary>
        /// <param name="sbyteArray">要轉換的 <see cref="sbyte"/> 字符數組</param>
        /// <returns>返回轉換後的 <see cref="byte"/> 字符數組</returns>
        ...
        /// <summary>
        /// 將此實例中的指定字符串轉換到 <see cref="byte"/> 字符數組。
        /// </summary>
        /// <param name="sourceString">要轉換的字符串</param>
        /// <returns>返回轉換後的 <see cref="byte"/> 字符數組</returns>
        ...
        /// <summary>
        /// 將此實例中的指定 <see cref="object"/> 數組轉換到 <see cref="byte"/> 字符數組。
        /// </summary>
        /// <param name="tempObjectArray">要轉換的 <see cref="object"/> 字符數組</param>
        /// <returns>返回轉換後的 <see cref="byte"/> 字符數組</returns>
        ...
        /// <summary>
        /// 將此實例中的指定 <see cref="byte"/> 字符數組轉換到 <see cref="sbyte"/> 字符數組。
        /// </summary>
        /// <param name="byteArray">要轉換的 <see cref="byte"/> 字符數組</param>
        /// <returns>返回轉換後的 <see cref="sbyte"/> 字符數組</returns>
        ...
        /// <summary>從流讀取字節序列,並將此流中的位置提升讀取的字節數.</summary>
        /// <param name="sourceStream">要讀取的流.</param>
        /// <param name="target">字節數組。此方法返回時,該緩衝區包含指定的字符數組,該數組的 start 和 (start + count-1) 之間的值由從當前源中讀取的字節替換。</param>
        /// <param name="start">buffer 中的從零開始的字節偏移量,從此處開始存儲從當前流中讀取的數據。.</param>
        /// <param name="count">要從當前流中最多讀取的字節數。</param>
        /// <returns>讀入緩衝區中的總字節數。如果當前可用的字節數沒有請求的字節數那麽多,則總字節數可能小於請求的字節數,或者如果已到達流的末尾,則為零 (0)。</returns>
        /// <exception cref="ArgumentException">start 與 count 的和大於緩衝區長度。</exception>
        /// <exception cref="ArgumentNullException">target 為空引用(Visual Basic 中為 Nothing)。</exception>
        /// <exception cref="ArgumentOutOfRangeException">offset 或 count 為負。</exception>
        /// <exception cref="System.IO.IOException">發生 I/O 錯誤。</exception>
        /// <exception cref="NotSupportedException">流不支持讀取。</exception>
        /// <exception cref="ObjectDisposedException">在流關閉後調用方法。</exception>
        ...
        /// <summary>從字符系列讀取字節序列,並將此字符系列中的位置提升讀取的字節數。</summary>
        /// <param name="sourceTextReader">要讀取的流。</param>
        /// <param name="target">字節數組。此方法返回時,該緩衝區包含指定的字符數組,該數組的 start 和 (start + count-1) 之間的值由從當前源中讀取的字節替換。</param>
        /// <param name="start">buffer 中的從零開始的字節偏移量,從此處開始存儲從當前流中讀取的數據。.</param>
        /// <param name="count">要從當前流中最多讀取的字節數。</param>
        /// <returns>讀入緩衝區中的總字節數。如果當前可用的字節數沒有請求的字節數那麽多,則總字節數可能小於請求的字節數,或者如果已到達流的末尾,則為零 (0)。</returns>
        ...
        /// 檢測當前文件的大小
        /// </summary>
        /// <param name="file">被檢測的文件</param>
        /// <returns>當前文件的大小。</returns>
