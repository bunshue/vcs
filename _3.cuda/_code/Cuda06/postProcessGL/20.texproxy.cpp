...
/* (int)floor(log2(x)) */
static int intFloorLog2(unsigned int x)
{
	int a = 0;
	while (x >>= 1)
	{
...
static GLboolean isPow2(unsigned int x)
{
	return ((x > 0) && (x & (x - 1) == 0));
}

/* find the largest texture with the specified internal format and aspect */
static void findMaxTexture(GLenum internalFormat, int dim, int xAspect, int yAspect, int border, int maxMipmapLevel, int* widthOut, int* heightOut)
{
