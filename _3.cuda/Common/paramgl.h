/*
   ParamListGL
   - class derived from ParamList to do simple OpenGL rendering of a parameter list sgg 8/2001
*/
        ...
        for (std::vector<ParamBase*>::const_iterator p = m_params.begin(); p != m_params.end(); ++p)
        {
            ...
            else
            {
                ...
                glRectf((GLfloat)(x + m_bar_x), (GLfloat)(y + m_bar_offset + m_bar_h),
                    (GLfloat)(x + m_bar_x + ((m_bar_w - 1) * (*p)->GetPercentage())), (GLfloat)(y + m_bar_offset + 1));
