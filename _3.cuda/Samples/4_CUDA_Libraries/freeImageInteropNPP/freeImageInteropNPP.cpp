...
// Detailed description of this example can be found as comments in the source code.
...
extern "C" void FreeImageErrorHandler(FREE_IMAGE_FORMAT oFif, const char* zMessage)
{
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        NPP_CHECK_CUDA(cudaMemcpy2D(pSrcImageCUDA, nSrcPitchCUDA, pSrcData, nSrcPitch, nImageWidth, nImageHeight, cudaMemcpyHostToDevice));
        ...
        const NppiSize oSizeROI = { (int)nImageWidth - (oMaskSize.width - 1),                               (int)nImageHeight - (oMaskSize.height - 1) };
        // allocate result image memory
        int nDstPitchCUDA;
        Npp8u* pDstImageCUDA = nppiMalloc_8u_C1(oSizeROI.width, oSizeROI.height, &nDstPitchCUDA);
        NPP_ASSERT_NOT_NULL(pDstImageCUDA);
        NPP_CHECK_NPP(nppiFilterBox_8u_C1R(pSrcImageCUDA, nSrcPitchCUDA, pDstImageCUDA, nDstPitchCUDA, oSizeROI, oMaskSize, oMaskAchnor));
        ...
        NPP_CHECK_CUDA(cudaMemcpy2D(pResultData, nResultPitch, pDstImageCUDA, nDstPitchCUDA, oSizeROI.width, oSizeROI.height, cudaMemcpyDeviceToHost));
