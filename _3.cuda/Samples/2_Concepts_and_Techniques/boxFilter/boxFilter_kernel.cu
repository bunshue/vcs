...
extern "C" void initTexture(int width, int height, void* pImage, bool useRGBA)
{
    ...
    checkCudaErrors(cudaMemcpy2DToArray(d_array, 0, 0, pImage, width * bytesPerElem, width * bytesPerElem, height, cudaMemcpyHostToDevice));
...
*/
extern "C" double boxFilter(float* d_src, float* d_temp, float* d_dest, int width, int height, int radius, int iterations, int nthreads, StopWatchInterface * timer)
{
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        if (iterations > 1)
        {
            ...
            checkCudaErrors(cudaMemcpy2DToArray(d_tempArray, 0, 0, d_dest, width * sizeof(float), width * sizeof(float), height, cudaMemcpyDeviceToDevice));
        }
    }
...
// RGBA version
extern "C" double boxFilterRGBA(unsigned int* d_src, unsigned int* d_temp, unsigned int* d_dest, int width, int height,
    int radius, int iterations, int nthreads, StopWatchInterface * timer)
{
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        {
            // copy result back from global memory to array
            checkCudaErrors(cudaMemcpy2DToArray(d_tempArray, 0, 0, d_dest, width * sizeof(unsigned int), width * sizeof(unsigned int), height, cudaMemcpyDeviceToDevice));
        }
    }
