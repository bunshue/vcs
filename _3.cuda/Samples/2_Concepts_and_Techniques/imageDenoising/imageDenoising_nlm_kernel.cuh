...
__global__ void NLM(TColor* dst, int imageW, int imageH, float Noise, float lerpC, cudaTextureObject_t texImage)
{
    ...
    if (ix < imageW && iy < imageH)
    {
            ...
            for (float j = -NLM_WINDOW_RADIUS; j <= NLM_WINDOW_RADIUS; j++)
            {
                ...
                for (float n = -NLM_BLOCK_RADIUS; n <= NLM_BLOCK_RADIUS; n++)
                    for (float m = -NLM_BLOCK_RADIUS; m <= NLM_BLOCK_RADIUS; m++)
                        weightIJ += vecLen(tex2D<float4>(texImage, x + j + m, y + i + n), tex2D<float4>(texImage, x + m, y + n));
                ...
                // Update weight counter, if NLM weight for current window texel exceeds the weight threshold
        ...
        // Choose LERP quotient basing on how many texels within the NLM window exceeded the weight threshold
...
////////////////////////////////////////////////////////////////////////////////
__global__ void NLMdiag(TColor* dst, unsigned int imageW, unsigned int imageH, float Noise, float lerpC, cudaTextureObject_t texImage)
{
    const int ix = blockDim.x * blockIdx.x + threadIdx.x;
    const int iy = blockDim.y * blockIdx.y + threadIdx.y;
    // Add half of a texel to always address exact texel centers
    ...
    if (ix < imageW && iy < imageH)
    {
        ...
        // Choose LERP quotient basing on how many texels within the NLM window exceeded the LERP threshold
