...
__global__ void d_filter_surface3d(int filterSize, float filter_offset, cudaExtent volumeSize, cudaTextureObject_t volumeTexIn, cudaSurfaceObject_t volumeTexOut)
{
...
extern "C" Volume * VolumeFilter_runFilter(Volume * input, Volume * output0, Volume * output1, int iterations, int numWeights, float4 * weights, float postWeightOffset)
{
    Volume* swap = 0;
    ...
    {
        d_filter_surface3d << <gridSize, blockSize >> > (numWeights, postWeightOffset, size, input->volumeTex, output0->volumeSurf);
