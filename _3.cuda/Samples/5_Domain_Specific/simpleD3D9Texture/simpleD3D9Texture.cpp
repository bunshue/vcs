...
int main(int argc, char* argv[])
{
    ...
    // Initialize Direct3D
    if (SUCCEEDED(InitD3D9(hWnd)) && SUCCEEDED(InitCUDA()) && SUCCEEDED(InitTextures()))
    {
        if (!g_bDeviceLost)
        {
...
HRESULT InitD3D9(HWND hWnd)
{
    ...
    if (FAILED(g_pD3D->CreateDeviceEx(g_iAdapter, D3DDEVTYPE_HAL, hWnd,
        D3DCREATE_HARDWARE_VERTEXPROCESSING, &d3dpp, NULL, &g_pD3DDevice)))
    {
        return E_FAIL;
    }
...
HRESULT InitTextures()
{
    ...
    if (FAILED(g_pD3DDevice->CreateTexture(g_texture_2d.width, g_texture_2d.height, 1, 0, D3DFMT_A32B32G32R32F, D3DPOOL_DEFAULT, &g_texture_2d.pTexture, NULL)))
    {
        return E_FAIL;
    }
    ...
    if (FAILED(g_pD3DDevice->CreateCubeTexture(g_texture_cube.size, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &g_texture_cube.pTexture, NULL)))
    {
        return E_FAIL;
    }
    ...
    if (FAILED(g_pD3DDevice->CreateVolumeTexture(g_texture_vol.width, g_texture_vol.height, g_texture_vol.depth, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &g_texture_vol.pTexture, NULL)))
    {
        return E_FAIL;
    }
    ...
    if (FAILED(hr = g_pD3DDevice->TestCooperativeLevel()))
    {
        ...
        if (hr == D3DERR_DEVICENOTRESET)
        {
            ...
            else
            {
                ...
                // This is a common function we use to restore all hardwareesources/state
                RestoreContextResources();
...
void RunCUDA()
{
    ...
    //   and to have the map/unmap calls be the boundary between using the GPU for Direct3D and Cuda
    //
