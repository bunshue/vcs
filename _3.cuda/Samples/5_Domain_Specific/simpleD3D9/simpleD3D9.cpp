...
HRESULT InitD3D9(HWND hWnd)
{
    ...
    if (FAILED(g_pD3D->CreateDeviceEx(g_iAdapter, D3DDEVTYPE_HAL, hWnd, D3DCREATE_HARDWARE_VERTEXPROCESSING, &g_d3dpp, NULL, &g_pD3DDevice)))
    {
        return E_FAIL;
    }
...
HRESULT InitVertexBuffer()
{
    ...
    if (FAILED(g_pD3DDevice->CreateVertexBuffer(g_NumVertices * sizeof(CUSTOMVERTEX), 0, D3DFVF_CUSTOMVERTEX, D3DPOOL_DEFAULT, &g_pVB, NULL)))
    {
        return E_FAIL;
    }
...
int main(int argc, char** argv)
{
    ...
    // and exit
    //
    printf("Running on %s exiting...\n", device_name);
    printf("Ssample finished returned: %s\n", (g_bPassed ? "OK" : "ERROR!"));
