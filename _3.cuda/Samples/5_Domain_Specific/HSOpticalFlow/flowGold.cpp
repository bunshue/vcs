...
///////////////////////////////////////////////////////////////////////////////
static void Downscale(const float* src, int width, int height, int stride, int newWidth, int newHeight, int newStride, float* out)
{
...
///////////////////////////////////////////////////////////////////////////////
static void Upscale(const float* src, int width, int height, int stride, int newWidth, int newHeight, int newStride, float scale, float* out)
{
    for (int i = 0; i < newHeight; ++i)
    {
        for (int j = 0; j < newWidth; ++j)
...
/// from a source image should be mapped to this particular output pixel.
...
static void SolveForUpdate(const float* du0, const float* dv0, const float* Ix,
    const float* Iy, const float* Iz, int w, int h, int s, float alpha, float* du1, float* dv1)
{
    for (int i = 0; i < h; ++i)
    {
        for (int j = 0; j < w; ++j)
...
void ComputeFlowGold(const float* I0, const float* I1, int width, int height,
    int stride, float alpha, int nLevels, int nWarpIters, int nSolverIters, float* u, float* v)
{
    ...
    for (; currentLevel < nLevels; ++currentLevel)
    {
        for (int warpIter = 0; warpIter < nWarpIters; ++warpIter)
        {
            ...
            // on current level we compute optical flow between frame 0 and warped frame 1
            ComputeDerivatives(pI0[currentLevel], tmp, pW[currentLevel], pH[currentLevel], pS[currentLevel], Ix, Iy, Iz);

            for (int iter = 0; iter < nSolverIters; ++iter)
            {
                SolveForUpdate(du0, dv0, Ix, Iy, Iz, pW[currentLevel], pH[currentLevel], pS[currentLevel], alpha, du1, dv1);
                Swap(du0, du1);
                Swap(dv0, dv1);
            }
        ...
        {
            // prolongate(©µªø¡F©Ôªø¡F©ì©µ) solution
            float scaleX = (float)pW[currentLevel + 1] / (float)pW[currentLevel];

            Upscale(u, pW[currentLevel], pH[currentLevel], pS[currentLevel], pW[currentLevel + 1], pH[currentLevel + 1], pS[currentLevel + 1], scaleX, nu);

            float scaleY = (float)pH[currentLevel + 1] / (float)pH[currentLevel];

            Upscale(v, pW[currentLevel], pH[currentLevel], pS[currentLevel], pW[currentLevel + 1], pH[currentLevel + 1], pS[currentLevel + 1], scaleY, nv);
    ...
    if (u != u0)
    {
        // solution is not in the specified array copy
    ...
    // cleanup
    // last level is not being freed here because it refers to input images
