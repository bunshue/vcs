...
int main(int argc, char* argv[])
{
    ...
    HWND hWnd = CreateWindow(wc.lpszClassName, "CUDA/D3D11 Texture InterOP", WS_OVERLAPPEDWINDOW, 0, 0,
    ...
    if (SUCCEEDED(InitD3D(hWnd)) && SUCCEEDED(InitTextures())) {
        ...
        // Create a buffer so that cuda can write into it pixel fmt is DXGI_FORMAT_R32G32B32A32_FLOAT
...
HRESULT InitD3D(HWND hWnd)
{
    ...
    // Setup the effect
    {
        ...
        hr = D3DX11CompileFromMemory(g_simpleEffectSrc, sizeof(g_simpleEffectSrc), "NoFile", NULL, NULL, "", "fx_5_0",
        ...
        hr = D3DX11CreateEffectFromMemory(effectCode->GetBufferPointer(), effectCode->GetBufferSize(), 0 /*FXFlags*/, g_pd3dDevice, &g_pSimpleEffect);
        AssertOrQuit(SUCCEEDED(hr));
...
void RunKernels()
{
    ...
    // populate the volume texture
    {
        ...
        cuda_texture_3d(g_texture_3d.cudaLinearMemory, g_texture_3d.width,
            g_texture_3d.height, g_texture_3d.depth, g_texture_3d.pitch, pitchSlice, t);
