...
int RunKernels(CudaContextData* currentContextData)
{
    ...
    // populate the 2d texture
    {
        ...
        cuda_texture_2d(currentContextData->cudaLinearMemory, g_texture_2d.width,
            g_texture_2d.height, g_texture_2d.pitch, g_bQAReadback ? 0.2f : t);
...
int Render()
{
    ...
    //
    {
        ...
        error = cudaD3D10GetDevices(NULL, &currentDevice, 1, g_pd3dDevice, cudaD3D10DeviceListCurrentFrame);
        printLastCudaError("cudaD3D10GetDevices failed");  // This prints and resets the cudaError to cudaSuccess
        AssertOrQuit(cudaSuccess == error);
        ...
        error = cudaD3D10GetDevices(NULL, &nextDevice, 1, g_pd3dDevice, cudaD3D10DeviceListNextFrame);
        printLastCudaError("cudaD3D10GetDevices failed");  // This prints and resets the cudaError to cudaSuccess
        AssertOrQuit(cudaSuccess == error);
