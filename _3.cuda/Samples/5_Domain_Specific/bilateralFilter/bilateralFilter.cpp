...
int runSingleTest(char* ref_file, char* exec_path)
{
    ...
    {
        printf("(radius=%d) (passes=%d) ", filter_radius, iterations);
        bilateralFilterRGBA(dResult, width, height, euclidean_delta, filter_radius, iterations, kernel_timer);
        ...
        cudaMemcpy2D(hResult, sizeof(unsigned int) * width, dResult, pitch, sizeof(unsigned int) * width, height, cudaMemcpyDeviceToHost);
...
void loadImageData(int argc, char** argv)
{
    ...
    LoadBMPFile((uchar4**)&hImage, &width, &height, image_filename);
