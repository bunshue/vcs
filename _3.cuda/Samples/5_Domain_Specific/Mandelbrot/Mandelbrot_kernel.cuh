...
// Unrolled version
template <class T>
__device__ inline int CalcMandelbrot(const T xPos, const T yPos, const T xJParam, const T yJParam, const int crunch, const bool isJulia)
{
    T x, y, xx, yy;
    int i = crunch;
    ...
    const float xJParam, const float yJParam, const int crunch, const bool isJulia)
{
    ...
    while (--i && (sum0 + sum1 < 4.0f))
    {
        ...
        dssub(x0, x1, xx0, xx1, yy0, yy1);  //  x = xx - yy + xC;  // xC is xPos for
                                            //  Mandelbrot and it is xJParam for Julia
...
// Increase the grid size by 1 if the image width or height does not divide evenly by the thread block dimensions
