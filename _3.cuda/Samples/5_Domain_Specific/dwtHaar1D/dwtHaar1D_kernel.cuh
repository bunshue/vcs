...
////////////////////////////////////////////////////////////////////////////////
__global__ void dwtHaar1D(float* id, float* od, float* approx_final, const unsigned int dlevels,
    const unsigned int slength_step_half, const int bdim)
{
    // Handle to thread block group
    cg::thread_block cta = cg::this_thread_block();
    ...
    if (dlevels > 1)
    {
        ...
        for (unsigned int i = 1; i < dlevels; ++i)
        {
            ...
            if (tid < num_threads)
            {
                ...
                // detail coefficient, not further modified so directly store in global memory
                ...
                // memory guarantees that the common representation (approx, detail_0, detail_1, ...) is achieved
