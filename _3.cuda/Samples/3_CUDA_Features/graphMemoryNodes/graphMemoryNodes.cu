 ...
 * free d_negSquare
 */
void createNegateSquaresGraphExplicitly(cudaGraphExec_t* graphExec, int device, negSquareArrays* hostArrays, float** d_negSquare_out = NULL)
{
    ...
    checkCudaErrors(cudaGraphAddMemcpyNode1D(&copyNodeInput, graph, &allocNodeSquare, 1, d_input, hostArrays->input,
    ...
    checkCudaErrors(cudaGraphAddMemcpyNode1D(
        &copyNodeSquare, graph, &squareKernelNode, 1, hostArrays->square, d_square, hostArrays->bytes, cudaMemcpyDeviceToHost));
    ...
    {
        printf("Check verified that d_negSquare and d_input share a virtual address.\n");
    }
    ...
    checkCudaErrors(cudaGraphAddMemFreeNode(&freeNodeSquare, graph, nodeDependencies.data(), nodeDependencies.size(), d_square));
    nodeDependencies.clear();

    checkCudaErrors(cudaGraphAddMemcpyNode1D(&copyNodeNegSquare, graph, &negateKernelNode, 1, hostArrays->negSquare,
...
void doNegateSquaresInStream(cudaStream_t stream1, negSquareArrays* hostArrays, float** d_negSquare_out = NULL)
{
    ...
    negateArray << <hostArrays->numBlocks, THREADS_PER_BLOCK, 0, stream1 >> > (d_square, d_negSquare, hostArrays->numElements);
    checkCudaErrors(cudaEventRecord(negateKernelCompleteEvent, stream1));
    checkCudaErrors(cudaMemcpyAsync(hostArrays->negSquare, d_negSquare, hostArrays->bytes, cudaMemcpyDeviceToHost, stream1));
    if (d_negSquare_out == NULL)
    {
 ...
 */
void createNegateSquaresGraphWithStreamCapture(cudaGraphExec_t* graphExec, negSquareArrays* hostArrays, float** d_negSquare_out = NULL)
{
    cudaGraph_t graph;
    cudaStream_t stream;
...
void prepareRefArrays(negSquareArrays* hostArrays, negSquareArrays* deviceRefArrays, bool** foundValidationFailure)
{
    ...
    checkCudaErrors(cudaMemcpy(deviceRefArrays->negSquare, hostArrays->negSquare, hostArrays->bytes, cudaMemcpyHostToDevice));
...
int main(int argc, char** argv)
{
    ...
    checkCudaErrors(cudaGraphLaunch(graphExec, stream));
    validateGPU << <hostArrays.numBlocks, THREADS_PER_BLOCK, 0, stream >> > (d_negSquare, deviceRefArrays, foundValidationFailure);
    ...
    printf("Validating negateSquares with d_negSquare freed outside the stream...\n");
    validateHost(&hostArrays, foundValidationFailure);
    checkValidationFailure(foundValidationFailure);
    resetOutputArrays(&hostArrays);
    ...
    checkCudaErrors(cudaStreamSynchronize(stream));
    printf("Validating negateSquares with d_negSquare freed in a different graph...\n");
    checkValidationFailure(foundValidationFailure);
