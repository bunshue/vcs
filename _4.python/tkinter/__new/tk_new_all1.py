...
class App:
    ...
    def initWidgets(self):
        #定義字符串元組
        books = ('Tkinter庫', '三大布局','Pack布局', 'Grid布局',\
            'Place布局')
...
def run(width=300, height=300):
    ...
    canvas.pack() 
    #設置事件
    window.bind("<Button-1>", lambda event:
...
class App:
    ...
    def initWidgets(self):
        #創建一個輸入組件
        self.show = tk.Label(relief=tk.SUNKEN, font=('Courier New', 24),\
            width=25, bg='white', anchor=tk.E)
        #對該輸入組件使用Pack布局，放在容器頂部
...
#按鈕
action = ttk.Button(window, text="單擊我", command=clickMe)  #創建一個按鈕, text：顯示按鈕上面顯示的文字, command：當這個按鈕被點擊之后會調用command函數
...
tk.Button(window, text='獲取信息', width=10, command=show).grid(row=3, column=0, sticky=W, padx=10, pady=5)
...
lab1=Label(mainfarm,text="你好，這是Checkbutton操作界面")
lab1.pack()
...
button2=tk.Checkbutton(mainfarm,
                    text='BUTTON2',
                    ...
                    padx=5, pady=5,  # 指定按鍵文本或圖像距離邊框的距離
                    relief=tk.RAISED,  # 指定按鍵的樣式
                    state=tk.ACTIVE,  # 指定按鍵的狀態
                    width=10, height=5,  # 制定按鍵的寬、高
                    )
button2.pack()
...
class App:
    ...
    def initWidgets(self):
        ...
        self.sb2 = Spinbox(self.master, 
            ...
            command = self.press) # 通過command綁定事件處理方法
...
s=tk.Scale(window,label='進行選擇',from_=5,to=11,orient=tk.HORIZONTAL,length=200,showvalue=1,tickinterval=3,resolution=0.01,command=print_selection)
...
ttk.Label(monty, text="Enter a name:").grid(column=0, row=0, sticky='W') #設置其在界面中出現的位置,column代表列,row代表行
#button被點擊之后會被執行
...
check1.grid(column=0, row=4, sticky=tk.W)       #sticky=tk.W(N：北/上對齊,S：南/下對齊,W：西/左對齊,E：東/右對齊)
chvarUn = tk.IntVar()
...
show = tk.StringVar()
Entry = tk.Entry(abc, textvariable=show, width="30")
Entry.pack() 
class section:
    def onPaste(self):
        try:
...
menu.add_command(label="復制", command=section.onCopy)
menu.add_separator()
menu.add_command(label="粘貼", command=section.onPaste)
menu.add_separator()
...
class App:
    ...
    def init_widgets(self):
        self.cv = tk.Canvas(root, background='white',
