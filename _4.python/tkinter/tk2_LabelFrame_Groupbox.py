...
for i in range(len(sports)):  # 將運動字典轉成核取方塊
    ...
    tk.Checkbutton(labFrame, text=sports[i], variable=checkboxes[i]).grid(
        row=i + 1, sticky=tk.W
    )
...
button1 = tk.Button(labelFrame1, text="紅色", width=8, command=fnRed).pack(
    side="left", padx=5
)
button2 = tk.Button(labelFrame1, text="綠色", width=8, command=fnGreen).pack(
    side="left", padx=5
)
button3 = tk.Button(labelFrame1, text="藍色", width=8, command=fnBlue).pack(
    side="left", padx=5
)
...
spinboxFourcc = tk.Spinbox(
    labelFrame1, value=("XVID", "DIVX", "MJPG", "I420"), width=10
)
