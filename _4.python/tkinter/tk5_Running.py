...
def display():
    ...
    def count():
        global counter  # 全域變數
        counter += 1
        lb_mesg1.config(text=str(counter), bg="pink", width=20, height=2)
...
progressbar1 = ttk.Progressbar(
    window, length=200, mode="determinate", orient=tk.HORIZONTAL
)
...
item1 = canvas.create_oval(
    10, 50, 60, 100, fill="yellow", outline="lightgray", tags="running_ball"
)
...
canvas.create_oval(
    x1 - radius, y1 - radius, x1 + radius, y1 + radius, fill="red", tags="moving1"
)

x2 = x1
y2 = 200
canvas.create_oval(
    x2 - radius, y2 - radius, x2 + radius, y2 + radius, fill="red", tags="moving2"
)
...
while running == True:
    ...
    if x2 < width:
        ...
        x2 = 0
    canvas.create_oval(
        x2 - radius, y2 - radius, x2 + radius, y2 + radius, fill="green", tags="moving2"
    )
