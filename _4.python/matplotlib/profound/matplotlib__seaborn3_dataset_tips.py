...
# 類別變數轉換為數值
tips.sex = tips.sex.map({"Female": 0, "Male": 1}).astype(int)
tips.smoker = tips.smoker.map({"No": 0, "Yes": 1}).astype(int)
tips.day = tips.day.map({"Thur": 1, "Fri": 2, "Sat": 3, "Sun": 4}).astype(int)
tips.time = tips.time.map({"Lunch": 0, "Dinner": 1}).astype(int)
...
tips["log_tip"] = np.log(tips["tip"])
sns.kdeplot(x="log_tip", data=tips)
plt.show()
...
plt.title("熱力圖 Correlation Matrix Heatmap")
plt.show()
...
sns.set()
sns.histplot(tips["total_bill"], kde=False)
sns.histplot(tips["total_bill"], kde=False, bins=20, color="red")
sns.histplot(tips["total_bill"], kde=False, bins=30, color="green")
...
sns.set()
sns.kdeplot(tips["total_bill"], label="default")
sns.kdeplot(tips["total_bill"], bw_adjust=2, label="bw_adjust: 2")
sns.kdeplot(tips["total_bill"], bw_adjust=5, label="bw_adjust: 5")
plt.legend()
plt.show()
...
sns.set()
sns.catplot(x="sex", y="total_bill", data=tips, kind="bar", col="day", col_wrap=2)
plt.show()
...
sns.barplot(x="day", y="total_bill", hue="sex", data=tips, ax=axs[0], capsize=0.1)
sns.barplot(
    x="tip", y="day", data=tips, ci=95, ax=axs[1]
...
"""
三、boxenplot
boxenplot是为更大的数据集绘制增强的箱型图。这种风格的绘图最初被命名为“信值图”，
因为它显示了大量被定义为“置信区间”的分位数。
...
通过增大百分比的粒度控制绘制的盒形图数目。
每个参数代表利用不同的统计特性对异常值的数量做出不同的假设。

scale：“linear” 或 “exponential” 或 “area”
用于控制增强箱型图宽度的方法。所有参数都会给显示效果造成影响。
“linear” 通过恒定的线性因子减小宽度，“exponential” 使用未覆盖的数据的比例调整宽度，
“area” 与所覆盖的数据的百分比成比例。

outlier_prop：float
被认为是异常值的数据比例。与 k_depth 结合使用以确定要绘制的百分位数。
...
"""
pointplot，如其名，就是点图。点图代表散点图位置的数值变量的中心趋势估计，
并使用误差线提供关于该估计的不确定性的一些指示。

点图比条形图在聚焦一个或多个分类变量的不同级别之间的比较时更为有用。
点图尤其善于表现交互作用：一个分类变量的层次之间的关系如何在第二个分类变量的层次之间变化。

重要的一点是点图仅显示平均值（或其他估计值），
但在许多情况下，显示分类变量的每个级别的值的分布可能会带有更多信息。
...
在估计值附近绘制置信区间的尺寸大小。如果是“sd”，则跳过引导阶段并绘制观察数据点的标准差。
如果为 None，则不会执行引导过程，并且不会绘制误差块。
...
# 用中位数作为集中趋势的估计
sns.pointplot(x="tip", y="day", data=tips, markers="h", estimator=np.median)
