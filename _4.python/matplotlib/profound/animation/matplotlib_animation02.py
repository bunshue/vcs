...
　　b.func自定義動畫函數，即下邊程序定義的函數animate

　　c.frames動畫長度，一次循環包含的幀數，在函數運行時，其值會傳遞給函數animate(n)的形參“n”
...
def animate(n):  #n就是 FuncAnimation 裡面的 frames
    print(n)
...
ani = FuncAnimation(fig,
                    animate,
                    frames = np.linspace(0, 2 * np.pi, 10),     #這里的frames在調用animate函數是會將frames作為實參傳遞給“n”
                    init_func = init,
                    interval = interval,
                    blit = True)
plt.show()
