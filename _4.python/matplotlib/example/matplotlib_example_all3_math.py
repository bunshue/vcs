...
dice_generator(N, sides)  # 產生擲骰子的串列
dice_count(sides)  # 將骰子串列轉成次數串列
NX = len(frequencies)
x = np.arange(NX)  # 長條圖x軸座標
width = 0.35  # 長條圖寬度

#plt.bar(x, frequencies, width, color="g")  # 繪製長條圖
plt.bar(x, frequencies, width, color='orange', hatch='o')  # 繪製長條圖, 多了hatch

plt.xlabel("骰子點數")
plt.ylabel("出現次數")
plt.title("測試" + str(N) + "次")
