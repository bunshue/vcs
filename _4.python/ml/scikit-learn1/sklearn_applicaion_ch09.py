...
cc = [i for i, j in enumerate(y_test) if j==0] == [i for i, j in enumerate(y_pred) if j==1]
print(cc)
...
# 存檔
plt.imsave('tmp_flower_kmeans.jpg', reconstruct_image(kmeans.cluster_centers_, labels, w, h))
...
#載入資料集
df = pd.read_csv('C:/_git/vcs/_big_files/Scikit-learn_data/invoice.csv', encoding="ISO-8859-1")
...
"""
# 依分數顯示客戶名單
cc = rfm_segmentation.sort_values(by=['RFMScore', 'Monetary'], ascending=[True, False]).head(10)
print(cc)
...
cc = rfm_segmentation.groupby('cluster')[['R_Quartile','F_Quartile', 'M_Quartile', 
                                     'Total_score']].mean()
print(cc)
