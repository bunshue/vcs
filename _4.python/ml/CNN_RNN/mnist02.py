...
train_feature_vector = train_feature.reshape(len(train_feature), 28, 28, 1).astype(
    "float32"
)
...
model.add(
    Conv2D(filters=20, kernel_size=(5, 5), padding="same", activation="relu")  # 建立卷積層2
)
...
model.compile(
    loss="categorical_crossentropy", optimizer="adam", metrics=["accuracy"]  # 定義訓練方式
)
...
model.save("tmp_Mnist_cnn_model.h5")  # 儲存模型
print("tmp_Mnist_cnn_model.h5 模型儲存完畢!")
