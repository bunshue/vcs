...
def text3(screen):
    ...
    if len(cars) > 10:
        cars = pd.read_excel(
            path + "停车场车辆表.xlsx", skiprows=len(cars) - 10, sheet_name="data"
        ).values
        # 动态绘制y点变量
        n = 0
        # 循环文档信息
...
if carnumber in carsk:
    ...
    # 更新停车场信息
    pi_info_tabel = pi_info_tabel.append(
        {"carnumber": carnumber, "date": localtime, "price": 3 * y, "state": 1},
        ignore_index=True,
    )
    # 保存信息更新.xlsx文件
    DataFrame(pi_talbe).to_excel(
        path + "停车场车辆表" + ".xlsx", sheet_name="data", index=False, header=True
    )
    DataFrame(pi_info_tabel).to_excel(
        path + "停车场信息表" + ".xlsx", sheet_name="data", index=False, header=True
    )
    # 停车场车辆
    carn -= 1
else:
    if carn <= Total:
        ...
        pi_talbe = pi_talbe.append(
            {"carnumber": carnumber, "date": localtime, "state": 0}, ignore_index=True
        )
        # 生成.xlsx文件
        DataFrame(pi_talbe).to_excel(
            path + "停车场车辆表" + ".xlsx", sheet_name="data", index=False, header=True
        ...
        if carn < Total:
            # state等于0时为停车场有车位的时候
            pi_info_tabel = pi_info_tabel.append(
                {"carnumber": carnumber, "date": localtime, "state": 0},
                ignore_index=True,
            )
            # 车辆数量+1
            carn += 1
            ...
            pi_info_tabel = pi_info_tabel.append(
                {"carnumber": carnumber, "date": localtime, "state": 2},
                ignore_index=True,
            )
            DataFrame(pi_info_tabel).to_excel(
                path + "停车场信息表" + ".xlsx", sheet_name="data", index=False, header=True
        ...
        if income_switch:
            ...
            for i in range(1, 13):
                ...
                if i < 10:
                    ...
                    # 筛选每月数据
                    kk = pi_info_tabel[
                        pi_info_tabel["date"].str.contains("2020-" + str(k))
                    ]
                    # 计算价格和
                    kk = kk["price"].sum()
                    v1.append(kk)
                    ...
                    for a, b in zip(attr, v1):
                        plt.text(
                            a, b, "%.0f" % b, ha="center", va="bottom", fontsize=7.5
                        )
                        # 设置柱状图标题
                        plt.title("每月收入统计")
                        # 设置y轴范围
