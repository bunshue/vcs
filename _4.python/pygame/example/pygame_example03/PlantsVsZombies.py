...
def AddRoles():
    ...
    if Add_Zombie_Total < Zombie_Total:  # 若果僵屍數量少於總數
        ...
        if Zombie_Rate_Counter == Zombie_Add_Rate:
            ...
            newZombie = {
                "rect": pygame.Rect(
                    UI_Width,
                    random.randint(10, UI_Height - zombieSize_Y - 10),
                    zombieSize_X,
                    zombieSize_Y,
                ),
                "surface": pygame.transform.scale(
                    zombieImage, (zombieSize_X, zombieSize_Y)
                ),
            }
        ...
        if Zombie2_Rate_Counter == Zombie_Add_Rate:
            ...
            newZombie2 = {
                ...
                "surface": pygame.transform.scale(
                    Zombie2Image, (zombie2Size_X, zombie2Size_Y)
                ),
            }
    ...
    if Bullet_Rate_Counter >= Bullet_Add_Rate and Shoot == True:
        ...
        newBullet = {
            "rect": pygame.Rect(
                EmitterRect.centerx + 10,
                EmitterRect.centery - 25,
                bulletRect.width,
                bulletRect.height,
            ),
            "surface": pygame.transform.scale(
                bulletImage, (bulletRect.width, bulletRect.height)
            ),
        }
...
rescaledBackground = pygame.transform.scale(
    backgroundImage, (UI_Width, UI_Height)
)  # 將背景圖縮放到游戲視窗大小
