...
class Cloud:
    ...
    def update(self):
        ...
        if self.x < -self.width:  # 若雲朵開始走道左邊視窗盡頭並消失
            self.x = window_width + random.randint(
                2500, 3000
            )  # 則將雲朵位置重新設在視窗外隨機800到1000
...
class Dinosaur:
    ...
    def run(self):
        self.image = self.run_img_list[
            self.step_index // 5
        ]  # 依 step_index 決定恐龍的跑步圖片，每五個step_index換一張圖
        self.dino_rect = self.image.get_rect()
        self.dino_rect.x = self.X_POS
        self.dino_rect.y = self.Y_POS
        self.step_index += 1
    ...
    def duck(self):
        self.image = self.duck_img_list[
            self.step_index // 5
        ]  # 依 step_index 決定恐龍的蹲下圖片，每五個step_index換一張圖
        self.dino_rect = self.image.get_rect()
        self.dino_rect.x = self.X_POS
...
class Bird(Obstacle):
    ...
    def draw(self, screen: pygame.display):
        if self.index >= 10:
            self.index = 0
        screen.blit(
            self.image_list[self.index // 5], self.rect
        )  # 以index決定飛行的動作，每五個index為一種飛行動作
        self.index += 1
...
def menu(death: bool):
    ...
    while run:
        ...
        if death == False:
            ...
            score_text = Text(
                "Your Score: " + str(points), 40, BLACK, score_text_position
            )
