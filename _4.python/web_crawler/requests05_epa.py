...
url = (
    "https://data.epa.gov.tw/api/v2/%s?format=%s&year_month=%s&offset=%s&limit=%s&api_key=%s&filters=%s"
    % (DataID, format, year_month, offset, limit, api_key, filters1)
)
print(url)

url = (
    "https://data.epa.gov.tw/api/v2/%s?format=%s&year_month=%s&offset=%s&limit=%s&api_key=%s&filters=%s"
    % (DataID, format, year_month, offset, limit, api_key, filters2)
)
print(url)

url = (
    "https://data.epa.gov.tw/api/v2/%s?format=%s&year_month=%s&offset=%s&limit=%s&api_key=%s&filters=%s"
    % (DataID, format, year_month, offset, limit, api_key, filters3)
)
print(url)
...
format = "csv"
url = "https://data.epa.gov.tw/api/v2/%s?format=%s&offset=%s&limit=%s&api_key=%s" % (
    DataID,
    format,
    offset,
    limit,
...
def downloadAQI():
    ...
    url = "https://data.epa.gov.tw/api/v2/%s?format=%s&limit=%s&api_key=%s" % (
        DataID,
        format,
        limit,
        api_key,
