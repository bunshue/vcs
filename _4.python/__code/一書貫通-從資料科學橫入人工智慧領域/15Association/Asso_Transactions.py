...
inverted=pd.read_csv(r'Transactions.csv')
inverted.head()
...
res.loc[res.lhs==frozenset({'Mountain-200'}),['rhs','support','confidence','lift']].sort_values('confidence',ascending=False).head(20)
...
res.loc[res.lhs==frozenset({'Mountain-200'}),['rhs','support','confidence','lift']].sort_values('lift',ascending=False).head(20)
