...
ax1.plot(x1, gamma1, lw=2, label=u"伽瑪分佈", color="red", alpha=0.6)
ax2.plot(x2, dist2,  lw=2, label=u"統計結果")
ax2.plot(x2, gamma2, lw=2, label=u"伽瑪分佈", color="red", alpha=0.6)

for ax, k in zip((ax1, ax2), ks):
    ax.set_xlabel(u"時間間隔")
    ax.set_ylabel(u"機率密度")
...
ax1.vlines(bus_time[:10], 0, 1, lw=2, color="blue", label=u"公共汽車車")
...
pl.plot(x, stats.chi2.pdf(x, 4), lw=2, label=u"$\chi ^{2}$分佈")
pl.plot(x, k(x), lw=2, color="red", alpha=0.6, label=u"樣本分佈")
pl.legend(loc="best")
pl.xlim(0, 10);
...
print(np.all(x2 == x3))
pl.plot(t, x, label=u"帶噪聲的訊號")
pl.plot(t, x2 + 0.5, alpha=0.6, label=u"中值濾波之後的訊號")
pl.legend(loc="best");
...
pl.subplot(211)
pl.plot(x1, y1, ".", label=u"資料點")
pl.plot(sx1, sy1, label=u"spline曲線")
pl.legend()

pl.subplot(212)
pl.plot(x2, y2, ".", label=u"資料點")
pl.plot(sx2, sy2, linewidth=2, label=u"spline曲線")
