...
strings = [
    "abcsub-033_ses-01_mod-mri",
    "defsub-044_ses-01_mod-mri",
    "ghisub-055_ses-02_mod-ctscan",
]
...
df = pd.DataFrame(
    [[name, obj.salary()] for name, obj in employees.items()],
    columns=["name", "salary"],
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", help="list of input files.", nargs="+", type=str
    )
    parser.add_argument(
        "-o",
        "--output",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i",
        "--input",
        help="list of input files or root directory",
        nargs="+",
        type=str,
    )
    ...
    parser.add_argument(
        "-p", "--pattern", help="list of input files or root directory", type=str
    )
    parser.add_argument(
        "-r", "--replacement", help="list of input files or root directory", type=str
    )
    ...
    if len(options.input) == 1 and os.path.isdir(options.input[0]):
        filenames = [
            ...
            for curdir, subdirs, files in os.walk(options.input[0])
            for file in files
        ]
    else:
        filenames = [f for f in options.input if os.path.isfile(f)]
