    ...
    def draw(Raphael):
        ...
        for x in range(ncols):
            for y in range(nrows):
                cell = paper.rect(
                    x0 + x * cellwidth, y0 + y * cellwidth, cellwidth, cellwidth
                )
...
def draw_nonogram(parameter):
    ...
    run_js = "(function(parameter){%s; draw_nonogram_js('%s', parameter);})(%s);" % (
        jscode,
        uid,
        parameter_js,
    )
...
def py2js_call(func, parameter):
    ...
    display_html('<div id="{}"></div>'.format(uid), raw=True)
    jscode = py2js(func)
    run_js = "(function(parameter){%s; %s('%s', parameter);})(%s);" % (
        jscode,
        func.__name__,
        uid,
        parameter_js,
