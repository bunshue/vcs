    ...
    def run(self):  # 進入監聽狀態
        while 1:  # 使用while循環不停監聽
            try:  # 捕捉例外
                ...
                self.edit.insert(tkinter.END, "連線來自:%s:%d\n" % addr)  # 向文字框中輸出狀態
                data = client.recv(1024)  # 接受資料
                self.edit.insert(tkinter.END, "收到資料:%s \n" % data)  # 向文字框中輸出資料
                client.send(str("I GOT: %s" % data).encode())  # 傳送資料
                client.close()  # 關閉同用戶端的連線
                self.edit.insert(tkinter.END, "關閉用戶端\n")  # 向文字框中輸出狀態
            except:  # 例外處理
                self.edit.insert(tkinter.END, "關閉連線\n")  # 向文字框中輸出狀態
                break  # 結束循環
...
class Window:  # 主視窗
    def __init__(self, root):
        self.root = root
        self.butlisten = tkinter.Button(root, text="開始監聽", command=self.Listen)  # 建立元件
        self.butlisten.place(x=20, y=15)
        self.butclose = tkinter.Button(root, text="停止監聽", command=self.Close)
