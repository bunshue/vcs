...
class OpenGLWindow:
	...
	def Draw(self):								# 繪制場景
		...
		glTranslatef(0.0, 0.0, -1.0)					# 搬移位置
		glColor3f(0.0, 1.0, 0.0)					# 設定彩色為綠色
		glRasterPos2f(0.0, 0.0)						# 定位文字
		self.DrawText('PyOpenGL')					# 繪制文字
		glutSwapBuffers()						# 交換快取
	def DrawText(self, string):						# 繪制文字函數
		for c in string:						# 循環處理字串
			glutBitmapCharacter(GLUT_BITMAP_8_BY_13, ord(c))	# 輸出文字
