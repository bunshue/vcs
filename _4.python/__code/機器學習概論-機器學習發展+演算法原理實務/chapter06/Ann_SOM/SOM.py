...
class SOM:
    def __init__(self, inputDim, mapWidth, mapHeight, inputPatternSet):
        ...
        self.__w = [
            [
                [random.random() * 0.1 for i in range(self.__patternLength)]
        ...
        for patIndex in range(len(patterns)):
            ...
            for i in range(self.__mapHeight):
                for j in range(self.__mapWidth):
                    ...
                    for k in range(3):
                        dif = (
                            self.__patterns[currentPatternIndex].patternItem[k]
                            - self.__w[i][j][k]
                        )
                        e = nbh * dif
    ...
    def __neighbourhood(self, i, j, centerI, centerJ, n):
        ...
        nbh = math.exp(
            -distance2 / (2.0 * math.pow(self.__sigma0 * math.exp(-n / self.__tau1), 2))
        )
