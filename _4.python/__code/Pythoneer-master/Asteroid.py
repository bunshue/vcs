...
# import simplegui  用 SimpleGUICS2Pygame 替代
...
class Ship:
    ...
    def shoot(self):
        ...
        forward_vector = angle_to_vector(self.angle)
        mis_pos = [
            self.pos[0] + forward_vector[0] * self.radius,
            self.pos[1] + self.radius * forward_vector[1],
        ]
        mis_vel = [
            self.vel[0] + 4 * forward_vector[0],
            self.vel[1] + 4 * forward_vector[1],
        ]
        a_missile = Sprite(
            mis_pos, mis_vel, 0, 0, missile_image, missile_info, missile_sound
        )
...
# key handlers
keydown_inputs = {
    "up": lambda: my_ship.set_thrust(True),
...
keyup_inputs = {
    "left": lambda: my_ship.inc_ang_vel(),
...
a_rock = Sprite(
    [WIDTH / 3, HEIGHT / 3], [1, 1], 0, ANGLE_VEL, asteroid_image, asteroid_info
)
a_missile = Sprite(
