...
public class PDFTextStripper extends PDFStreamEngine
{
    ...
    private boolean overlap( TextPosition tp1, TextPosition tp2 )
    {
    	...
    	float diff = (tp1.getHeight() + tp2.getHeight())/5;
        ...
        if( showCharacter )
        {
            ...
            else
            {
            	...
            	for(int i=0;i<textList.size();++i)
            	{
            		...
            		TextPosition previousTextPosition = (TextPosition)textList.get(i);
            		
            		if(previousTextPosition.getCharacter().equals(text.getCharacter()))
            		{
            		/*System.out.println("overlapping test:"+
            				text.getCharacter()+previousTextPosition.getX() 
            				+ " " +text.getX()+" "+ previousTextPosition.getY()
            				+" " +text.getY());*/
            		
	                if(overlap(previousTextPosition,text))//isDiacritic(text) && 
	                {
	                    //previousTextPosition.mergeDiacritic(text);
	                    overlapping = true;
	                    //System.out.println("overlapping 1");
	                }
	                else
	                if(previousTextPosition.contains(text))
	                {
	                    previousTextPosition.mergeDiacritic(text);
	                	//System.out.println("contain overlapping 1");
	                    overlapping = true;
	                }
	                /* If the previous TextPosition was the diacritic, merge it into this
	                 * one and remove it from the list. */
	                else if( text.contains(previousTextPosition))//isDiacritic(previousTextPosition) &&
	                {
	                	System.out.println("contain overlapping 2");
	                    text.mergeDiacritic(previousTextPosition);
	                    textList.remove(textList.size()-1);
	                    textList.add(text);
	                    overlapping = true;
	                    //System.out.println("overlapping 2 "+previousTextPosition.getCharacter());
	                }
            		}
            	}
     ...
     */
    public boolean isDiacritic(TextPosition text) 
    {
