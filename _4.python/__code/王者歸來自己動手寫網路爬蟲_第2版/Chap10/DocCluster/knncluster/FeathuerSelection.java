		...
		for( i=0;i<list.length;i++){
			...
			for(int k=0;k<ret.size();++k)
	      {
			...
			for(j=0;j<wordlist.length;j++){
				if(wordset.IsStopWord(wordlist[j])||symboset.IsStopWord(wordlist[j].substring(0,1))||wordlist[j].length()<2||(wordlist[j].charAt(0)>='вп'&&wordlist[j].charAt(1)<='в╕')||((wordlist[j].charAt(0)>='0'&&wordlist[j].charAt(1)<='9'))){
					continue;
				}
