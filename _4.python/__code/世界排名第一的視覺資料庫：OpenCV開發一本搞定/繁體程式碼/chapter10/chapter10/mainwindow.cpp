...
void MainWindow::on_NEmbed_triggered()
{
    //非盲水印，最低有效位，內嵌過程
    //讀取載體圖形
    QString filename = QFileDialog::getOpenFileName(this,tr("開啟載體圖形"),"",tr("Image File(*.bmp *.jpg *.jpeg *.png)"));
    QTextCodec *code = QTextCodec::codecForName("gb18030");
    std::string name = code->fromUnicode(filename).data();
    srcImage = cv::imread(name);
    if(!srcImage.data)
    ...
    //只能內嵌二進位值圖形，若果是彩色圖形需要先轉換成灰階圖形再處理為二進位值圖形
    //cvtColor( wmImage,wmImage, COLOR_RGB2GRAY );
    Mat wmImageShow;  //該Mat專門用來顯示用。
    // threshold(wmImage,wmImageShow, 127, 255, THRESH_BINARY );
    wmImage.copyTo(wmImageShow);
    //設定值處理，通常水印圖形中只有0和255兩種值，將其中的255處理為1.
    threshold(wmImage,wmImage, 127, 1, THRESH_BINARY );   //保留最低位有效位LSB即可。
    //分析矩陣，用於分析最低有效位
    Mat t(MM,NN,CV_8UC1,1);
    t.copyTo(embedSrc);
    //透過逐位元與運算，將bgr[0]的最低有效位分析出來
    cv::bitwise_and(bgr[0],t,embedSrc);
    //透過逐位元互斥動作，將原始水印與得到載體圖形R通道的最低有效位進行運算，得到內嵌訊息embedSrc.
    cv::bitwise_xor(embedSrc,wmImage,embedSrc);
    //分析矩陣，用於分析高7位
    Mat t2(MM,NN,CV_8UC1,254);
    Mat t3;
    t2.copyTo(t3);
    //透過逐位元與運算分析bgr[0]的高7位
    cv::bitwise_and(bgr[0],t2,t3);
    //透過將載體圖形的高7位，與得到的只有一位的內嵌訊息進行逐位元或，完成將內嵌訊息embedSrc內嵌到載體圖形的R通道內。
...
void MainWindow::on_NExtract_triggered()
{
    ...
    //*****************************分析含水印圖形的R通道最低有效位****************************************************
    //讀取含水印圖形
    filename = QFileDialog::getOpenFileName(this,tr("開啟含水印載體圖形"),"",tr("Image File(*.bmp *.jpg *.jpeg *.png)"));
    name = code->fromUnicode(filename).data();
...
void MainWindow::on_NBRembed_triggered()
{
    ...
    */
    ...
    //內嵌水印
    //將特定位與水印訊息進行比較。
    //內嵌水印是0時，不需要動作即完成水印的內嵌。
    //內嵌水印是1時，需要判斷內嵌位的值決定如何運算。
    int flag=0;
    int n;
    for(i=0;i<M;i++)
        for(j=0;j<N;j++)
