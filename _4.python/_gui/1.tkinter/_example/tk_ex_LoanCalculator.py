...
tk.Entry(window, textvariable=annualInterestRateVar, justify=tk.RIGHT).grid(
    row=1, column=2
)
...
lblMonthlyPayment = tk.Label(window, textvariable=monthlyPaymentVar).grid(
    row=4, column=2, sticky=tk.E
)
totalPaymentVar = tk.StringVar()
lblTotalPayment = tk.Label(window, textvariable=totalPaymentVar).grid(
    row=5, column=2, sticky=tk.E
)
btComputePayment = tk.Button(
    window, text="Compute Payment", command=computePayment
...
class LoanCalculator:
    def __init__(self):
        ...
        btComputePayment = Button(
            window, text="Compute Payment", command=self.computePayment
