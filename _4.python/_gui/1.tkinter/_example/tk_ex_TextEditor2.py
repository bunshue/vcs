...
class text_editor:
    ...
    def tsize(self):
        ...
        s = Scale(
            t, from_=0, to=100, orient=HORIZONTAL, length=100, width=10, sliderlength=20
        )
        ...
        Button(
            t, text="Apply Changes", font=("arial black", 10), command=change_size
        ).pack()
    ...
    def open_func(self):
        returned = filedialog.askopenfile(
            initialdir="E:\\", title="Select file to open"
        )
        if returned != None:
    ...
    def new(self):
        msg = messagebox.askyesnocancel(
            "title", "All the Progress will be not saved.\n Do you want to save."
        )
        if msg == True:
            self.save_func()
            self.text_box.delete(1.0, END)
    ...
    def more(self):
        ...
        Label(
            more, text="Text Editor", bg="powder blue", font=("Arial Black", 10, "bold")
        ).pack()
        Label(
            more,
            text="By: Harsh Bardhan Mishra",
            bg="powder blue",
            font=("Arial Black", 10, "bold"),
        ).pack()
        Label(
            more,
            text="This Text Editor was created as a hobby project just for time-pass.",
            bg="powder blue",
            font=("Arial Black", 10, "bold"),
        ).pack()
        Label(
