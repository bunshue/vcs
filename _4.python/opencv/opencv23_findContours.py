...
image1 = cv2.imread(filename)  # 彩色讀取

image_gray1 = cv2.cvtColor(image1, cv2.COLOR_BGR2GRAY)  # 轉灰階

# 圖片先處理方法一
image_gray1 = cv2.GaussianBlur(image_gray1, (13, 13), 0)  # 執行高斯模糊化
edged = cv2.Canny(image_gray1, 50, 150)
contours, hierarchy = cv2.findContours(
...
"""
#圖片先處理方法二
ret,thresh = cv2.threshold(image_gray1,127,255,0)
# 二值化處理影像
# thresh = 127  # 定義閾值, 閾值以上為全白255, 閾值以下為全黑0
# ret, dst_binary = cv2.threshold(image_gray, thresh, maxval, cv2.THRESH_BINARY)
...
# 繪製圖形外輪廓
dst = cv2.drawContours(src, contours, -1, GREEN, 2)  # 繪製圖形外輪廓

# 填滿圖形外輪廓
# dst = cv2.drawContours(src, contours, -1, GREEN, -1)  # 繪製圖形外輪廓
...
dst = cv2.drawContours(mask, contours, -1, GREEN, -1)  # 繪製圖形外輪廓
...
dst = cv2.drawContours(src, contours, -1, GREEN, 3)  # 繪製圖形外輪廓
...
src = cv2.imread("data/findContours/3shapes.jpg")  # 彩色讀取
src = cv2.imread("data/findContours/myheart.jpg")  # 彩色讀取
...
src1 = cv2.imread("data/findContours/mycloud1.jpg")  # 彩色讀取
contours, hierarchy = get_image_contours(src1)
cnt1 = contours[0]

src2 = cv2.imread("data/findContours/mycloud2.jpg")  # 彩色讀取
contours, hierarchy = get_image_contours(src2)
cnt2 = contours[0]

src3 = cv2.imread("data/findContours/explode1.jpg")  # 彩色讀取
...
src = cv2.imread("data/findContours/explode1.jpg")  # 彩色讀取
src = cv2.imread("data/findContours/explode2.jpg")  # 彩色讀取
...
print(f"轉換後的矩形頂角 = \n {points}")
dst = cv2.drawContours(src, [points], 0, GREEN, 2)  # 繪製圖形外輪廓
...
src = cv2.imread("data/findContours/explode3.jpg")  # 彩色讀取
src = cv2.imread("data/findContours/explode1.jpg")  # 彩色讀取
...
src = cv2.imread("data/findContours/cloud.jpg")  # 彩色讀取
...
src = cv2.imread("data/findContours/multiple.jpg")  # 彩色讀取
...
src = cv2.imread("data/findContours/unregular.jpg")  # 彩色讀取
...
src = cv2.imread("data/findContours/heart1.jpg")  # 彩色讀取
src = cv2.imread("data/findContours/hand1.jpg")  # 彩色讀取
...
src = cv2.imread("data/findContours/hand2.jpg")  # 彩色讀取
...
src = cv2.imread("data/findContours/star.jpg")  # 彩色讀取
...
src = cv2.imread("data/findContours/star1.jpg")  # 彩色讀取
...
src = cv2.imread("data/findContours/simple.jpg")  # 彩色讀取
...
src = cv2.imread("data/findContours/hand.jpg")  # 彩色讀取  # 手上有一黑點與一白點
...
cv2.drawContours(image, coin_contours, -1, RED)
...
image = cv2.imread("data/nested_patterns.png")  # 彩色讀取
