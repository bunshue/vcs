...
filename2 = "C:/_git/vcs/_4.python/_data/lena_color.jpg"
...
for p in points:
    print(p)
    cv2.circle(image, (p[0], p[1]), 10, BLUE, -1)  # 繪製圓形

points = np.intp(points)  # 轉為整數座標

# cv2.drawContours() #多點頭尾連線
cv2.drawContours(image, [points], 0, RED, 5)

# cv2.polylines()  # 空心多邊形
cv2.polylines(image, [points], True, GREEN, 1)  # True表示封口
# True: 頭尾相連, False: 頭尾不相連
