	...
	for x in range(heigh):
		for y in range(wide):
			if x < heigh//2 and y < wide//2:
				#对两张图片低频的地方进行权值融合数据
				reImgA[x,y,c] = weightImgA[x,y,c]*waveImgA[x,y,c] + weightImgB[x,y,c]*waveImgB[x,y,c] 
			else:
				#对两张图片高频的进行绝对值系数最大规则融合
				reImgA[x,y,c] = waveImgA[x,y,c] if abs(waveImgA[x,y,c]) >= abs(waveImgB[x,y,c]) else waveImgB[x,y,c] 
	...
	for y in range(wide):
		for x in range(heigh):
			if x%2 == 0:
				...
				reImgB[x,y,c] = reImgA[x//2,y,c] - reImgA[x//2 + heigh//2,y,c] 
			else:
